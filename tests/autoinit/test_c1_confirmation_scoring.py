...
def test_the_committed_equivalence_record_says_identical():
    rec = json.loads((REPO / "logs/stages/stage-1/phase_c1/results/scoring_equivalence.json").read_text())
...
def test_the_equivalence_gate_cannot_cover_correct_implies_usable(monkeypatch,
    ...
    recorded in `logs/stages/stage-1/phase_c1/results/scoring_equivalence.json`.
    """
    def mutate(mp):
