...
from aadistill.initialization.specs.state import OperatorStep  # noqa: E402
...
def test_the_feasibility_floor_is_seed_aware_and_has_an_absolute_guard():
    from aadistill.initialization.planning.recovery import FeasibilityRule
...
def test_the_catastrophic_capability_rule_is_enforced_at_both_rungs():
    ...
    from aadistill.initialization.planning.recovery import CapabilitySchema
...
def test_the_catastrophic_rule_is_part_of_the_plan_hash():
    from aadistill.initialization.planning.recovery import CatastrophicCapabilityRule
...
def test_the_aggregation_is_part_of_the_frozen_plan_identity():
    from aadistill.initialization.planning.recovery import SeedAggregation
...
def _protocol(**overrides):
    from aadistill.initialization.planning.recovery import RecoveryProtocolFingerprint
...
def test_a_matched_pair_is_same_protocol_same_seed_different_init():
    from aadistill.initialization.planning.recovery import RecoveryProbeIdentity
...
def test_the_availability_report_does_not_claim_a_matched_control():
    ...
    path = REPO / "logs/shared/analyses/autoinit_control_availability.json"
    if not path.is_file():
...
def test_every_preflight_stop_condition_is_a_whole_string():
    ...
    from aadistill.initialization.planning.recovery import PreflightStage
