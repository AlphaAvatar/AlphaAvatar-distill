...
def test_the_probe_sees_the_sessions_this_repository_can_actually_launch():
    ...
    assert kinds >= {"phase_a", "phase_b", "recovery_continuation", DEFAULT_KIND}
...
@pytest.mark.parametrize("kind", ["phase_a", "phase_b", "recovery_continuation"])
...
def test_the_dedicated_branches_do_not_share_a_type():
    dedicated = {k: code_only(BRANCHES[k])
                 for k in ("phase_a", "phase_b", "recovery_continuation")}
    classes = {k: re.findall(r"(\w+Authorization)\.load\(", v)
               for k, v in dedicated.items()}
    flat = [c for v in classes.values() for c in v]
    assert len(flat) == len(set(flat)) == 3, classes
