...
def test_the_historical_attestation_is_read_not_rewritten():
    ...
    import subprocess
    for rel in ("logs/stages/stage-1/phase_a/results/autoinit_stage3_complete/attested_evaluation_protocol.json",
                "logs/stages/stage-1/phase_a/results/autoinit_stage3_complete/materialized_thresholds.json",
                "logs/stages/stage-1/phase_a/results/autoinit_stage3_complete/engine_probe.json"):
        diff = subprocess.run(["git", "diff", "--", rel], cwd=REPO,
                              capture_output=True, text=True).stdout
        assert diff == "", f"{rel} was modified; it is historical evidence"
    att = json.loads((REPO / "logs/stages/stage-1/phase_a/results/autoinit_stage3_complete"
...
def test_a_real_content_digest_is_material_in_full(tmp_path):
    ...
    protocol = json.loads(
        (REPO / "logs/stages/stage-1/phase_a/results/autoinit_stage3_complete/attested_evaluation_protocol.json")
