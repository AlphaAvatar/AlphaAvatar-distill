...
| `autoinit_c2_behavioural_launch.py` | **Phase C2 behavioural selection** session specification — twelve probes, two rungs, one verdict. `SESSION_KIND=c2_behavioural`. The only C2 session that TRAINS, so its `BehaviouralAuthorization` is the only one reporting `allows_recovery_training` True; it reports `authorizes_c2_full_search`, `authorizes_c2_search1`, `authorizes_c2_baseline_completion`, `authorizes_c2_replay` and `authorizes_later_cycles` all False, and its own `load` refuses a document claiming any of them — it cannot delegate to `C2Authorization.load`, which refuses `allows_recovery_training` by construction. Seven `$0` gates; `destination_gate` is the one with no counterpart elsewhere — it verifies the off-pod store has room for twelve 1.11 GiB probes BEFORE a pod exists, because C1 attempt 18 ran a correct preservation mechanism on six probes and preserved none when every upload was refused for quota. It stages only small assets: the six 1.19 GB arms are BUILT on the pod along digest-pinned paths, since `scp` gives each local asset 600 s against an uplink needing ~1650 and the hub relay refuses 5.95 GB for private-storage quota. `on_poll` secures each finished probe off-pod during the run and re-identifies it against all six identity fields; teardown is refused while the evidence is UNREADABLE, because "I found no probes" and "no probes were trained" are different findings |
...
| `autoinit_c2_behavioural_driver.py` | the **Phase C2 behavioural selection**: twelve probes in two rungs, and the only evidence that may name a C2 incumbent. Stages P/S/R/C/D -- prepare, screen, rank, confirm, decide. It SUBCLASSES `C1Driver` rather than rebuilding the training loop, because a harness that reimplemented the loop could not notice the loop being broken; `train_one` and the battery scorer stay the seams C1 built for `$0` regressions, and only `descriptors`, the rung boundary and the selection are overridden. Stage P is the one that does not exist in C1: the five candidates are staged durable products, but incumbent B is not -- baseline-completion attempt 8 preserved its evidence and not its bytes -- so B is materialized from the frozen C1 treatment path and gated on its exact identity, and if that gate fails NO SCREENING PROBE MAY START. Screening ranks and advances exactly one candidate and `assert_screening_emits_no_verdict` refuses a screening result carrying a verdict, an incumbent or a promotion; confirmation cannot begin until all six screening probes are trained AND scored. Each probe is persisted as it completes |
...
| `batching_refactor_cuda_launch.sh` | the operator-topology/batching refactor's focused real-CUDA check. One pod, one job, teardown in an `EXIT` trap. Its four subruns are recorded in `logs/stages/stage-1/phase_c3/validations/batching-refactor-cuda/v1/campaign.json`; the finding it produced was returned to review as insufficiently attributed, and the launcher is kept because that finding's provenance runs through it |
| `engineering_campaign_budget.py` | what ANY engineering campaign may spend on one session, derived from the records that own each part of it: the ceiling from the investigation's `authorization.json`, the costs from its `campaign.json`. Extracted from the launcher's bash so it can be wrong for free — inline, the only way to learn what it computed was to create a pod. Recomputes spend from components, floors the session limit, and reports `ok: no` rather than letting the caller re-decide. Named for what it does, not for the first campaign that used it: it was `batch_invariance_budget.py` until the C3 pilot needed it, and a second campaign reading a file named for the first is how a number gets read out of the wrong book |
| `batch_invariance_diagnostic_launch.sh` | the batch-invariance root-cause diagnostic's launcher. Carries **no budget constant** and does no budget arithmetic: `engineering_campaign_budget.py` above derives the session limit and this script obeys it, refusing on `ok: no`. Ships its pod payload as a rendered FILE rather than an inline heredoc, because an unquoted heredoc is expanded by the local shell and has already run a `pip install` on the dev box |
| `batch_invariance_diagnostic_remote.sh` | that diagnostic's pod side. Builds the environment the **formal science** runs in — `/opt/train`, python 3.12, torch 2.11.0+cu128 and transformers 5.13.1 installed offline from the relay wheelhouse — and then measures under the image's own torch 2.9.1+cu130 as well, because which of this repository's three runtimes a number came from is exactly what the rejected a4 finding could not say. Stages the frozen calibration mixture from the relay against its pinned hash: `artifacts/` is gitignored, so without it the diagnostic would silently measure random token ids |

| `c3_batching_pilot_launch.sh` | the C3 batching-adoption pilot's launcher. Same shape as the diagnostic launcher above and two deliberate differences: the L40S **securePrice is re-queried live immediately before creation** and every derived bound — session ceiling, deadline, watchdog, ledger — is built from that number rather than from `L40S_MEASURED.price_per_hour_usd`, which is historical throughput-profile provenance; and if the live rate makes the `$5.00` ceiling infeasible it **stops** rather than shrinking the scientific work. `--volumeSize 0`: no network volume, because no stage consumes one |
| `c3_batching_pilot_remote.sh` | that pilot's pod side. Builds ONLY the formal science environment — one pinned runtime, because the adoption gate is a ratio of two wall clocks and two runtimes would make it a ratio of two different things. Fetches the root teacher at the **pinned revision** the frozen path names, never `main`. Runs the driver's own `--toy` sequence as a seconds-long preflight first: the three process-global registries it needs are empty in a fresh interpreter, two were missing until a toy run found them, and each failure surfaced only after the stage before it had succeeded |
| `c3_batching_pilot_driver.py` | the pilot itself, as one owned sequence: replay the frozen prefix **once** at B=1, verify `eea90c91…`, then score both arms from that one parent as verified suffixes. Every stage persists as it completes, because a pilot that dies in its second arm still measured the first. `--toy` runs the identical sequence at a CPU geometry against a synthetic root — the same pilot functions, the same operator, the same comparator, the same committed records, nothing stubbed |
