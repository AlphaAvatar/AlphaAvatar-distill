...
| `autoinit_c1_launch.py` | **Phase C1** session specification: fixed-path ATTENTION isolation. `SESSION_KIND=c1`, `C1Authorization`, a `BudgetSpec` derived from `logs/phase_c1_pricing.json`, and ten pre-provider gates: the ninth proves a pod can obtain the exact authorized commit from the relay, the tenth that the shared setup's `ROPE_OK` step has the 1,418-byte config it globs for. Runs no search |
