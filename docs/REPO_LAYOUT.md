...
| `scripts/experiments/phase_c1/authorization.py` | **Phase C1**: the `C1Authorization` type (hard-`False` `allows_phase_a`/`allows_beam_search`, refused by schema at load), the declared C1 harness file set, and `c1_budget_spec` — which derives the enforceable ceiling from `logs/stages/stage-1/phase_c1/plans/phase_c1_pricing.json` so it exists in one place |
...
| `src/aadistill/initialization/operators/attention/gqa/` | the grouped-query-attention **topology**: `weight_proxy.py`, `activation_importance.py`, the shared selection topology in `_common.py`, and the per-head second moment in `_statistics.py`. A topology, not a model family — an adapter declaring `ATTENTION_GQA` reaches these whatever family it is. **Import is inert**: `activation_importance` registers only when a consumer calls `register()`, because an unrestricted `BeamSearch` enumerates the whole registry |
| `src/aadistill/initialization/operators/{ffn/dense,width/residual,depth,composite}/` | the other kinds, same rule: a topology directory exists only where there is a real structural distinction, which is why DEPTH has no artificial topology level |
| `src/aadistill/initialization/calibration/batching.py` | padded micro-batches for calibration-item forwards, and the means to take each item's own result back out. Right padding, explicit mask, per-item reduction preserved |
| `src/aadistill/initialization/execution.py` | `ExecutionConfig` — HOW an operator runs. Separate from the `config` that is hashed into the state id, so a batch size cannot fork a scientific identity |
