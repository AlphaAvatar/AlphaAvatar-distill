# AutoInitializer reference

**Durable reference, not state.** Binding rules, pinned assets, protocol
requirements, measurement constraints and implementation status for the
AutoInitializer line of work. It changes when the *design* changes, not when a
run finishes.

Live state is elsewhere and this file must not restate it:

* [`logs/state/current.json`](../logs/state/current.json) — the machine snapshot
* [`logs/state/current.md`](../logs/state/current.md) — its human view
* [`logs/budget/ledger.md`](../logs/budget/ledger.md) — spend and caps

Moved here from `logs/state/current.md` on 2026-08-18, unchanged, because STATE.md is a
snapshot and this is reference material that was making it 1,100 lines long.
...
single-variable experiment.** [decisions](../logs/budget/decisions.md) 2026-08-10.
...
| epsilon-Pareto beam policy + hashable width schedule | `src/aadistill/autoinit/ranking.py` | level 0 prunes nothing; NLL is not an objective; lineage diversity across fronts; single-objective needs an explicit acknowledgement |
| shard-safe checkpoint identity | `src/aadistill/autoinit/artifact.py` | digest over every shard + index + config + arch + tokenizer; frozen single-file hashes stay checkable |
...
4. **Build `calib.reasoning_heavy@v1`** for Phase B — a reweighted draw from the
   same pool, zero cost. It is currently representable but not built, and
   `resolve()` refuses it. **Phase B's trigger is independent of Phase A's
   outcome**: order and operator-specific calibration are separate hypotheses.
