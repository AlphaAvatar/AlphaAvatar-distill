...
A paid training session lost its structured training logs to a bundling step
that listed an audit directory and two JSON files — the set the session it was
copied from had needed — and never listed the training event stream, because
that earlier session did not train. `tar` bundled what it was given, exited 0, the
...
that construct, inside frozen records of completed runs. It is banned going
forward, and this module is the replacement.
...
# required because it is the one that was lost, and because it is the only artifact
...
def create_archive(manifest: ArtifactManifest, out_path: str | Path) -> Path:
    ...
    tarball — that difference is the entire artifact loss above.
...
GATE_ORDER: tuple[str, ...] = (
    ...
    # the products this session OWES off-pod actually secured? A paid session
    # staged every selected leaf on the pod, fetched none of them because the
    # product fetch returned early when a later stage failed, and lost them — with
