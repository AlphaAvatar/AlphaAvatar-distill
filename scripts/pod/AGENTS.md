...
2. Wait for readiness. **Do NOT trust `runpodctl pod get`'s `uptimeSeconds`: in
   ...
   **`runpodctl ssh info` returning `"pod not ready"` does not mean the pod
   ...
   "stuck" (`logs/stages/stage-3/history/EXPERIMENTS.md`).
...
paid for four infrastructure lessons (`logs/stages/stage-3/history/EXPERIMENTS.md` §9):
...
outside a git checkout (P4 gap, `logs/stages/stage-3/history/EXPERIMENTS.md` §10). Ship the commit hash
