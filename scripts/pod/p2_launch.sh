...
BACKSTOP_MINUTES=${BACKSTOP_MINUTES:-330}
...
$SSH "root@$HOST" "cd /workspace && SESSION_COMMIT=$SESSION_COMMIT \
  ...
  TEACHER_REVISION=$TEACHER_REVISION HOLDOUT_SRC=/workspace/aad_holdout/holdout_v1.jsonl \
  bash /workspace/p2_setup.sh" \
  >>"$SCR/p2_setup.log" 2>&1
if [ $? -ne 0 ]; then
  say "FATAL: setup failed. $(cost)"; tail -25 "$SCR/p2_setup.log" | tee -a "$LOG"
