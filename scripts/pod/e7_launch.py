#!/usr/bin/env python3
"""Dev-box orchestrator for the Experiment 7 pod.
...
teardown gate.
    ...
    PYTHONPATH=src setsid nohup python -u scripts/pod/e7_launch.py \
...
WS = "/workspace"
REPO = f"{WS}/aad"
STATUS = f"{WS}/e7.status"
RUN_LOG = f"{WS}/e7_run.log"
ARMS = ("e7_fineweb_r1600k_sa", "e7_fineweb_r1600k_sb",
        "e7_control_r1600k_sa", "e7_control_r1600k_sb")
STEP = "step_001761"
...
class E7:
    ...
    def say(self, msg: str) -> None:
        ...
        with open(self.scr / "e7_launch.log", "a") as f:
            f.write(line + "\n")
    ...
    def make_plan(self) -> bool:
        self.plan = plan_session(
            ...
            arms=4, steps_per_arm=1761,
            step_time=StepTime(4.60, "E6b measured 4.15 s/step + 10% for the "
                                     "extra stream"),
            setup_minutes=45.0, eval_minutes_per_arm=10.5,
            transfer_minutes=40.0,
            other_phases=(Phase("arm_A_general_text_diagnostics", 4.0),
    ...
    def setup_on_draw(self, draw: int) -> str:
        ...
        subprocess.run(scp + [str(REPO_ROOT / "scripts/pod/e7_setup.sh"),
        ...
        target.run(
            ...
            f"bash {WS}/e7_setup.sh > {WS}/e7_setup.log 2>&1; "
            f"echo SETUP_RC=$? >> {WS}/e7_setup.log",
        ...
        probe_out = target.run(
            ...
            f"tail -1 {WS}/e7_setup.log", timeout=120).stdout.splitlines()
        ...
        if cold not in ("", "0"):
            return "cold"
        if done in ("", "0"):
            tail = target.run(f"tail -40 {WS}/e7_setup.log", timeout=120).stdout
    ...
    def run(self) -> bool:
        ...
        job = start_detached(target, JobSpec(
            job_id="e7_driver", workdir=REPO,
            command=(f"/opt/train/bin/python scripts/pod/e7_driver.py --stage all "
        ...
        ) + (RelaySpec(RUN_LOG, "e7_run.log", required=False),
             RelaySpec(STATUS, "e7.status", required=False))
    ...
    def collect_and_teardown(self, target, host, scp, job) -> bool:
        ...
        target.run(f"mkdir -p {REPO}/artifacts/audit/e7_session && "
                   f"cp {RUN_LOG} {STATUS} {REPO}/artifacts/audit/e7_session/",
                   timeout=120)
        man = f"{WS}/e7_manifest.json"
        ...
        if (store / "manifest.json").is_file():
            ...
            try:
                with tarfile.open(store / "e7_artifacts.tar.gz") as tar:
        ...
        ck = target.run(
            f"cd {REPO}/artifacts/stage3 && find e7_*_r1600k_*/checkpoints/{STEP} "
        ...
        state = {
            ...
            "transfer_complete": (store / "e7_artifacts.tar.gz").is_file(),
...
def main() -> int:
    ...
    ap.add_argument("--ckpt-store", default="/home/ecs-user/aad-artifacts/e7")
    ...
    ap.add_argument("--poll-limit-min", type=float, default=740.0)
    ...
    ap.add_argument("--out", default="logs/experiments/early/analyses/e7_session_evidence.json")
    args = ap.parse_args()
    ...
    try:
        ...
        e7.ev["driver_error"] = f"{type(exc).__name__}: {exc}"
        e7.say(f"LAUNCHER ERROR: {type(exc).__name__}: {exc}")
        if e7.pod_id:
            e7.teardown_now("launcher error")
    e7.ev["passed"] = bool(ok)
    e7.ev["finished_utc"] = datetime.now(timezone.utc).isoformat(timespec="seconds")
    e7.save()
    print(f"\nE7 session {'COMPLETE' if ok else 'INCOMPLETE'} — "
