#!/usr/bin/env bash
# Setup for the 2026-08-04 diagnostic pod: padding-truncation benchmark plus the
# two reference/recall diagnostics. One pod, two stacks:
#
#   * training venv (uv sync against the committed lockfile -> transformers
#     5.13.1) for the benchmark and the teacher-forced top-1 pass;
#   * a separate vLLM venv on the CONTAINER disk for generation.
#
# transformers 5.x matters here and is not incidental: it moved rope_theta into
# a nested `rope_parameters` dict, and a 4.x reader silently resolves our
# checkpoints to the class default 10000 instead of the stored 5000000. Both
# venvs are checked below, and the run refuses to start on a skewed one.
#
...
# --- data: the E1 pack and the source corpus ------------------------------
...
# --- training env ---------------------------------------------------------
say "training env via uv sync (cu128 torch, transformers from the lockfile)"
...
# --- models ---------------------------------------------------------------
# The control checkpoint is pushed from the dev box by the launcher (it exists
# nowhere else -- the relay holds only its evaluation JSONs).
say "staging the Stage 1 init and downloading the reference + teacher"
python3 - <<'PY'
import os, shutil
from pathlib import Path
...
snapshot_download("Qwen/Qwen3-0.6B", revision="$REF_REVISION", token=tok,
                  allow_patterns=["*.json","*.safetensors","*.jinja","*.txt"])
snapshot_download("Qwen/Qwen3-4B-Thinking-2507", revision="$TEACHER_REVISION",
                  token=tok,
                  allow_patterns=["*.json","*.safetensors","*.jinja","*.txt"])
print("reference and teacher downloaded")
PY
python3 - <<'PY'
import hashlib, sys
