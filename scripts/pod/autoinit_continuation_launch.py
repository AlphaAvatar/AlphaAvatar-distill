...
CONTROL_RECORDS = tuple(
    f"logs/stages/stage-1/phase_a/results/autoinit_permanent_controls/{c}_{suffix}.json"
    for c in CONTROLS
...
def materialize_controls(ctx: SessionContext) -> bool:
    ...
    for name in CONTROLS:
        ...
        for suffix in ("run_manifest", "run_completion"):
            subprocess.run(
                scp + [str(REPO_ROOT / "logs/stages/stage-1/phase_a/results/autoinit_permanent_controls"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--out", default="logs/stages/stage-1/recovery_continuation/analyses/autoinit_continuation_session.json")
    return ap
