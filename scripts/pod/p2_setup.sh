...
say "verifying the single-variable guarantee"
/opt/train/bin/python -c "
import json
for t in ('sa','sb'):
    a=json.load(open(f'/workspace/aad/configs/stage3/e1/e1_r0860k_{t}_pca.json'))
    b=json.load(open(f'/workspace/aad/configs/stage3/p2/p2_ceheavy_{t}.json'))
    ld={k for k in set(a['loss'])|set(b['loss']) if a['loss'].get(k)!=b['loss'].get(k)}
    assert ld=={'ce_weight','kd_weight'}, (t, ld)
    assert b['loss']['kd_scope']=='all' and a['loss']['kd_scope']=='all'
    assert (b['loss']['ce_weight'],b['loss']['kd_weight'])==(1.0,0.25)
    assert b['loss']['kd_temperature']==a['loss']['kd_temperature']
    assert 'truncate_padding' not in b['batch']
    for k in ('student_path','rung','packing','block_len','seed','trainable_patterns',
              'optim','schedule','batch','teacher','dtype','autocast_bf16',
              'gradient_checkpointing','val_blocks'):
        assert a.get(k)==b.get(k), (t,k,a.get(k),b.get(k))
    print(f'  {t}: ce_weight/kd_weight are the ONLY differing fields; kd_scope unchanged')
"
mark SETUP_DONE
say "setup complete"
