#!/usr/bin/env bash
# Setup for the Experiment 7 pod: FineWeb teacher-KD mixture at the 1.60M rung.
#
# Four arms train from the Stage 1 PCA init — B (FineWeb KD) and C (matched
# in-domain KD control), two seeds each. Arm A is NOT retrained: its two
# checkpoints are staged from the relay so the general-text diagnostics can be
...
# The extra-KD streams are hash-verified against `logs/e7_relay_manifest.json`,
...
#          -> INIT_READY -> STREAMS_READY -> ARMA_READY -> TEACHER_READY
#          -> ROPE_OK -> TESTS_OK -> MASK_OK -> ARMS_VALIDATED -> SETUP_DONE
set -euo pipefail
...
# --- the E7 treatment and its matched control ------------------------------
# These three streams ARE the experiment. Their hashes are pinned in the
# committed relay manifest; a mismatch means the arm would not be the arm the
# preregistration describes, so it is fatal here rather than discovered in the
# analysis.
say "staging the E7 extra-KD streams"
python3 -c "
import sys; sys.path.insert(0, '/workspace')
from fetch import fetch
...
from pathlib import Path
relay = json.loads(Path('logs/e7_relay_manifest.json').read_text())
for name, entry in relay['streams'].items():
    ...
    print(f'  {name}: {entry["n_blocks"]}x{entry["block_len"]}, '
          f'{entry["kd_positions"]:,} KD positions, pad {entry["padding_tokens"]}')
b = relay['streams']['e7_fineweb_kd']
c = relay['streams']['e7_control_kd']
...
# --- arm A, retained, not retrained ----------------------------------------
say "staging the retained arm A checkpoints (baseline, NOT retrained)"
python3 -c "
import sys; sys.path.insert(0, '/workspace')
from fetch import fetch
for seed in ('sa', 'sb'):
...
for seed, sha in want.items():
    ...
    if h != sha:
        sys.exit(f'ARM A {seed} HASH MISMATCH: {h} != {sha}')
print('retained arm A checkpoints verified against the inventory')
PYEOF
mark ARMA_READY
...
mark SETUP_DONE
say "setup complete — four arms will train from the Stage 1 init"
