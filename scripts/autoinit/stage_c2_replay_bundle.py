...
stages through its own module because the record it writes is the one that
session's `bundle_staged_gate` reads; a bundle staged through another session's
stager lands on the shared relay under the right name carrying a record the gate
...
def main(argv=None) -> int:
    ...
    #: The stage id comes from this run's own layout, not from a config
    #: document: the replay has no separate authorization config, because its
    #: plan IS the source binding and a second document would be a second place
    #: to edit.
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            if args.dry_run:
                built = build_bundle(REPLAY_TRANSPORT, REPO_ROOT, commit,
        ...
        record = {
            "schema": "aadistill.autoinit.c2_replay_bundle/v1",
            "run_id": args.run_id,
            "stage_id": stage_id,
            "session_commit": commit,
            "canonical_bundle_name": built["canonical_name"],
            "relay_repo": REPLAY_TRANSPORT.relay_repo,
            ...
            "upload": built["upload"],
            "verify": built["verify"],
            "transport": REPLAY_TRANSPORT.label,
