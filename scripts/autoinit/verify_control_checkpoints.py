"""Verify the canonical 0.86M control checkpoints exist and are the right ones.
...
This runs *before* the micro-preflight is priced, because the answer decides
whether the preflight is a profiling job or a profiling job plus two 0.86M
recovery runs. It does not trust the tombstone's "full copy on the relay" claim —
...
def main() -> None:
    ...
    for name, meta in CONTROLS.items():
        ...
        if entry["present_on_relay"]:
            ...
            entry["matches_intended_control_protocol"] = bool(
                entry["weights_hash_match"] and entry["config_hash_match"]
                and entry["lineage_valid"])
        else:
            entry["matches_intended_control_protocol"] = False
        all_ok = all_ok and entry["matches_intended_control_protocol"]
        report["controls"][name] = entry

    report["both_controls_available"] = all_ok
    report["retrieval_bytes"] = sum(
        e.get("size_bytes") or 0 for e in report["controls"].values())
    report["consequence"] = (
        "Both canonical 0.86M controls are retrievable and verified. The "
        "micro-preflight stays a profiling/evaluation job: no recovery retraining "
        "is needed, and the two-seed pooled control characterization can be "
        "produced by evaluating these checkpoints on recovery_search_v1."
        if all_ok else
        "At least one canonical control is missing or does not verify. Do NOT "
        "redefine the threshold to one seed. Rerun the canonical control from "
        "qwen3_0p6b_init_v0 at 0.86M for both seeds under the frozen recipe; "
        "those runs become the permanent Phase-A control probes and must be "
        "retained, not treated as disposable setup work.")
    report["report_sha256"] = sha256_json(report)

    (REPO_ROOT / args.out).write_text(json.dumps(report, indent=2) + "\n")
    print(json.dumps({
        "both_controls_available": all_ok,
