    ...
    doc = {
        ...
        "campaign": {
            ...
            "handoff": {
                ...
                "cost": (
                    ...
                    "of them are 22.2 GiB. Copied onto the pod during the "
                    "session, over the dev box's measured 0.49-0.91 MB/s "
                    "uplink, that was about 585 minutes of BILLED time, "
                    "because an scp to a pod happens after the pod exists -- "
                    "and at that price it was unaffordable against what the "
                    "campaign has left. RESOLVED 2026-09-23 in two steps. "
                    "First the probes were pre-staged onto a provider network "
                    "VOLUME, written once through a $0.06/h CPU pod with every "
                    "file re-hashed from the bytes that landed, so a later pod "
                    "could ATTACH them instead of receiving them and the "
                    "reserve fell to re-identification. Then, under AGENTS.md "
                    "P8.4, the weights stopped moving entirely: all ten "
                    "completed probes are validly scored, nothing remaining "
                    "reads a checkpoint, and the verdict consumes rows and "
                    "scores. The working set is 8.1 MiB of evidence against "
                    "22.21 GiB -- 0.035% -- the reserve is 10 minutes, and no "
                    "volume is attached, which also unpins the draw from the "
                    "volume's single datacenter. The uplink is a HOST cap rather than a "
                    "per-connection one -- parallel streams starve each other "
                    "rather than adding -- so no arrangement of transfers "
                    "during the session would have made this affordable. An "
                    "object store was built and never run: every one reachable "
                    "from here needs an account only a maintainer can create, "
                    "and the Hugging Face account that exists has 2.62 GiB of "
                    "private headroom against 22.2 GiB needed."),
            },
        ...
        "candidate_transport": {
            ...
            "_why_not_staged": (
                ...
                "against a dev-box uplink measured between 0.49 and 0.91 MB/s "
                "across every observation this programme has taken of it. "
                "Recovery-continuation attempt 2 "
