...
def main() -> None:
    ...
    isolation = json.loads((REPO_ROOT / "logs/shared/analyses/autoinit_role_isolation.json").read_text())
    thresholds = json.loads(
        (REPO_ROOT / "logs/shared/analyses/autoinit_threshold_characterization.json").read_text())
    ...
    prereg = {
        ...
        "role_isolation": {
            "report": "logs/shared/analyses/autoinit_role_isolation.json",
        ...
        "canonical_control_availability": json.loads(
            (REPO_ROOT / "logs/shared/analyses/autoinit_control_availability.json").read_text())
            if (REPO_ROOT / "logs/shared/analyses/autoinit_control_availability.json").is_file()
            else "NOT VERIFIED",
        "recovery_identity": {
        ...
        "tool_scoring_audit": json.loads(
            ...
            if (REPO_ROOT / "logs/shared/analyses/autoinit_tool_scoring_audit.json").is_file()
            else None,
