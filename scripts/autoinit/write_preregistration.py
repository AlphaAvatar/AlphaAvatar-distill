...
companion prose document is `logs/experiments/phase_a/analyses/autoinit_phase_a_preregistration.md`.
...
DRAFT_PREREGISTRATION = "logs/experiments/phase_a/plans/autoinit_phase_a_preregistration.json"
MATERIALIZED_PREREGISTRATION = (
    "logs/experiments/phase_a/plans/autoinit_phase_a_preregistration_materialized.json")
...
FROZEN_RECOVERY_PLAN = "logs/experiments/phase_a/analyses/autoinit_phase_a_recovery_plan_frozen.json"
...
def build_frozen_plan(repo_root: str | Path = REPO_ROOT, *,
                      ...
                      stage3: str = "logs/experiments/phase_a/results/autoinit_stage3_complete",
                      ) -> SuccessiveHalvingPlan:
...
def main() -> None:
    ...
    ap.add_argument(
        "--stage3", default="logs/experiments/phase_a/results/autoinit_stage3_complete",
    ...
    isolation = json.loads((REPO_ROOT / "logs/experiments/shared/analyses/autoinit_role_isolation.json").read_text())
    thresholds = json.loads(
        (REPO_ROOT / "logs/experiments/shared/analyses/autoinit_threshold_characterization.json").read_text())
    ...
    prereg = {
        ...
        "role_isolation": {
            "report": "logs/experiments/shared/analyses/autoinit_role_isolation.json",
        ...
        "canonical_control_availability": json.loads(
            (REPO_ROOT / "logs/experiments/shared/analyses/autoinit_control_availability.json").read_text())
            if (REPO_ROOT / "logs/experiments/shared/analyses/autoinit_control_availability.json").is_file()
            else "NOT VERIFIED",
        "recovery_identity": {
        ...
        "recovery_scoring_contract": {
            **recovery_scoring_contract(REPO_ROOT),
            "validation": json.loads(
                (REPO_ROOT / "logs/experiments/recovery_continuation/analyses/autoinit_recovery_scoring_validation.json"
                 ).read_text())
            if (REPO_ROOT / "logs/experiments/recovery_continuation/analyses/autoinit_recovery_scoring_validation.json"
            ...
            "supersession": {
                ...
                "history": ("both records are kept; logs/budget/decisions.md carries the "
        ...
        )(json.loads((REPO_ROOT / "logs/experiments/recovery_continuation/analyses/autoinit_recovery_fingerprint_audit.json"
                      ).read_text()))
        if (REPO_ROOT / "logs/experiments/recovery_continuation/analyses/autoinit_recovery_fingerprint_audit.json").is_file()
        else "NOT AUDITED",
        "tool_scoring_audit": json.loads(
            (REPO_ROOT / "logs/experiments/shared/analyses/autoinit_tool_scoring_audit.json").read_text())
            if (REPO_ROOT / "logs/experiments/shared/analyses/autoinit_tool_scoring_audit.json").is_file()
            else None,
        ...
        "pending_before_launch": [
            item for item, satisfied in (
                ...
                ("a PhaseAAuthorization artifact issued against the rehearsed "
                 ...
                 (REPO_ROOT / "logs/budget/approvals/autoinit_phase_a_authorization.json").is_file()),
        ...
        "satisfied_before_launch": [
            ...
            "(logs/experiments/phase_a/analyses/autoinit_phase_a_repricing.md)",
        ],
    }
