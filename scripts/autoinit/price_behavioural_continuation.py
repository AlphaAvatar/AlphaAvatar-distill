...
"""What the Phase-B behavioural continuation still owes. Zero cost; launches nothing.
    ...
    PYTHONPATH=src python scripts/autoinit/price_behavioural_continuation.py \
        --out logs/shared/analyses/autoinit_behavioural_continuation_pricing.json
...
def price(hardware=L40S_MEASURED) -> dict:
    ...
    return {
        ...
        "reuse_provenance": {
            ...
            "records": [
                {"record": "logs/shared/analyses/autoinit_historical_probe_reuse.json",
                 "probes_dir_digest": json.loads(HISTORICAL.read_text())["probes_dir_digest"],
                 "admits": sorted(ev["historical"])},
                {"record": "logs/shared/analyses/autoinit_attempt5_probe_reuse.json",
                 "probes_dir_digest": json.loads(ATTEMPT5.read_text())["probes_dir_digest"],
                 "admits": sorted(ev["attempt5"])},
                {"record": "logs/shared/analyses/autoinit_attempt4_probe_reuse.json",
...
def main() -> None:
    ...
    ap.add_argument("--out", default="logs/shared/analyses/autoinit_behavioural_continuation_pricing.json")
    args = ap.parse_args()
    r = price()
    out = Path(args.out)
