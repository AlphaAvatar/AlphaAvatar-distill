...
| --- | --- | --- | --- | --- | --- | --- |
| qwen3-4b-thinking-distill (working name) | `Qwen/Qwen3-4B-Thinking-2507` @ `768f209d` | 0.6B-class: hidden 1024, 28L, FFN 3072, 16Q/8KV, tied emb (user-chosen 2026-07-13) | **stage3-running** | Stage 0, Stage 1, Stage 2 (v0+v1); Stage 3 sub-stages 1–2; Experiments 1–6 complete | **`e1_r2960k_{sa,sb}_pca`** (E6, 2026-08-08) — usable rollout **0.8400**, correct **0.2067**; +0.1067 usable over the former best `e4_p2_r1600k_{sa,sb}`, above the 0.0800 floor and seed-consistent, tying it on correctness. Tied with `e1_r5500k` on behaviour and preferred on the registered correctness tie-break at half the tokens. Lowest teacher-native CE remains `e1_r5500k_sa_pca` (1.0032), a diagnostic and not a selection identity | Termination is what scale fixes: context-limit hits 28/44 → 19/23 prompts, GSM8K usable 0.71 → 0.92 — while **GSM8K correctness stays 0.00–0.01**. Stage 3 still not exited: no reasoning at any rung, and no prospectively registered gate exists to pass. See below. |
