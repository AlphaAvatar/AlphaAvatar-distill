...
| `autoinit_c1_session.json` | the **live C1 session record**, rewritten by each launch; it currently holds **attempt 5**: 12/12 pre-provider gates PASS, pod `e2ghwfuerdrvni` created at `$1.09/h`, setup reached ROPE_OK and then failed the CPU test suite (`2 failed, 2769 passed, 99 skipped`), `SETUP_RC=1`, pod deleted at 17.34 min for `$0.3150`, `provider_confirms_gone: true`. **No scientific stage ran** in any C1 attempt. Each attempt's copy is preserved under its own `autoinit_c1_attemptN*/`. The attempt-1 record it once held read: all eight pre-provider gates PASS, pod `fccr23o9jcnrh0` created, then ABORT at setup draw 1 with `SETUP_RC=1` — the pod could not fetch `transfer/c1`, which does not exist on the relay because no git bundle was created for this session's commit. **No scientific stage ran**; no teacher fetch, no replay, no training, no evaluation. `$0.0786`, pod deleted at 4.8 min, `provider_confirms_gone: true`. The authorization's one launch attempt is consumed |
...
| `autoinit_c1_bundle.json` | the local half of the C1 transport check, written by `stage_c1_bundle.py`: session commit, canonical bundle name, relay path, sha256 and bytes. `bundle_staged_gate` verifies the REMOTE object against it. The working copy records the Attempt-5 bundle `aad_autoinit_79c18adb.bundle` (7,669,955 bytes, `2d1865ae7973…`); it is **intentionally uncommitted** — staging it during a launch window would add a third path to the session lineage diff and `session_commit_gate` would refuse — so the committed copy still reads Attempt 3R. Attempt 5's copy is preserved in `autoinit_c1_attempt5/` |
...
| `autoinit_c1_attempt3r/` | evidence for **C1 attempt 3R**, the repriced execution: the `$0` ten-gate rehearsal and the script that produced it, the launcher log, the watchdog journal, the pod id `pj3c870n6yx70z`, the consumed Attempt-3R authorization, the bundle record and the session record. Setup reached VLLM_READY → TEACHER_READY → **ROPE_OK** — both previously fatal gaps closed and stayed closed — then failed the CPU test suite (`14 failed, 2650 passed`). **No scientific stage ran.** `$0.3482`, 18.85 min, provider confirms gone |
| `autoinit_c1_attempt4/` | preserved raw evidence for **C1 attempt 4**, the first attempt to launch on a launch-bound readiness record: the launcher log with all six failing nodeids, the independent pod poll, the watchdog journal ending on `pod_gone`, the session record, the consumed authorization, and `test_gate_failures.json` — the complete enumeration with each cause and the single root cause (the sweep ran with the simulator's DEFAULT hidden set, not C1's staged set). 12/12 pre-provider gates passed, pod `d854jag1ybhny2` was created at `$1.09/h`, setup reached `ROPE_OK` and died at the CPU test gate. `$0.6986`, 38.46 min, provider confirms gone. **No scientific stage ran** |
| `c1_storage_recovery.json` | the **2026-09-06 storage recovery**, under a limited P12 approval covering rebuildable caches and ephemeral residue only. Inventory before, every deletion with its measured and freed bytes and why it was safe, what was protected and why, what was large but left alone as outside the approval, and the byte-identity proof for the one scratch checkpoint removed. `0.64 GiB → 12.01 GiB` free. **No canonical checkpoint or scientific artifact was deleted**, and `aad-artifacts` is byte-for-byte unchanged |
| `c1_skip_predicate_audit.json` | **every skip predicate in the C1-SELECTED suite**, with the signal its condition reads and whether a pod sees the same premise. Written after attempt 5 died on a guard keyed to the SIMULATOR's `AAD_SYNTHETIC_HF_TOKEN`. Path premises are RESOLVED against the git index and the SetupManifest rather than guessed from keywords; whatever cannot be resolved must carry an explicit entry in [`configs/autoinit/c1_skip_predicate_classification.json`](../configs/autoinit/c1_skip_predicate_classification.json), and a registered line that no longer holds a predicate is reported STALE. Regenerate with `scripts/autoinit/audit_skip_predicates.py --write`; a test requires the committed digest to match the live tree |
| `autoinit_c1_attempt5/` | preserved raw evidence for **C1 attempt 5**: the launcher log with both failing nodeids, the independent pod poll, the watchdog journal, the pod id, the session record, the consumed authorization, the launch-bound readiness record, the bundle record, and `test_gate_failures.json` — the enumeration with each cause, the root cause (a dev-box-only skip guard keyed on the SIMULATOR's `AAD_SYNTHETIC_HF_TOKEN` flag, which the pod does not set, so two tests that the sweep correctly skipped ran on the pod and failed) and the one skip divergence that remains UNEXPLAINED. 12/12 pre-provider gates passed and the launcher re-ran them all before creating anything; pod `e2ghwfuerdrvni` was created at `$1.09/h`, setup reached `ROPE_OK` and died at the CPU test gate with `2 failed, 2769 passed, 99 skipped`. `$0.3150`, 17.34 min, provider confirms gone. **No scientific stage ran** |
| `autoinit_c1_attempt6/` | preserved raw evidence for **C1 attempt 6**: the launcher log, the independent pod poll, the watchdog journal ending on `pod_gone`, the pod id, the session record, the consumed authorization, the launch-bound readiness record, the bundle record, **`pod_pytest_outcomes.json` pulled OFF THE POD before teardown** (36,728 chars: all 113 skip nodeids with reasons, all 18 failures, the computed set difference) and `test_gate_failures.json`. 12/12 pre-provider gates passed and the launcher re-ran them all; pod `n71opk7lv4fhzf` was created at `$1.09/h` and the strict skip-set comparison REFUSED at the CPU gate, naming two expected-skip-but-ran and one pod-only skip. `$0.3665`, 20.17 min, provider confirms gone. **No scientific stage ran** |
| `autoinit_c1_attempt7/` | preserved raw evidence for **C1 attempt 7**, the first attempt ever to clear the pod CPU test gate: the launcher log, the independent poll, the watchdog journal, the artifact manifest, the session record, the consumed authorization, the launch-bound readiness record, the bundle record, `relay/` (the COMPLETE marker sequence and the driver's own stderr, relayed off the pod while it ran) and `failure.json`. `MARKER:TESTS_OK:861s` then the driver exited 2 on `unrecognized arguments: --stage all` -- a root cause CONFIRMED from the failing process's output, not attributed. Pod `gfd8buh5tr51qb`, `$0.4231`, 23.29 min, provider confirms gone. **No scientific stage ran** |
| `autoinit_c1_attempt7_grant.json` | the **maintainer decision permitting ONE C1 session -- attempt 7**, at the attempt-6 postmortem review of 2026-09-06. A NEW artifact; the six prior C1 grants were each consumed by one issuance. Records that attempt 6 established two accepted facts on a real L40S -- the command-scoped CPU-test environment hid CUDA and restored the real runtime, and the strict skip-set comparison correctly refused three real divergences -- and that all four repairs are accepted. States only maintainer-owned fields: `$265.7679` of the unchanged `$283.7600` cap, and the twenty-one things it does not authorize, retry and replacement pod first. The `$2.8446` ceiling-case remainder is NOT authorization for another run. Asserts no derived identity |
| `autoinit_c1_attempt6_grant.json` | the **maintainer decision permitting ONE C1 session -- attempt 6**, at the CPU-test parity review of 2026-09-06. A NEW artifact; the five prior C1 grants were each consumed by one issuance. States only maintainer-owned fields: who permitted what, at `$265.4014` of the unchanged `$283.7600` cap, and the twenty-one things it does not authorize -- including any retry or replacement pod. Records that the `$3.2111` remaining after one full ceiling is NOT authorization for another attempt, and that an early termination still requires a completely new review. Carries the canonical ten-step issuance ORDER whose first step is the launch-bound sweep on the frozen pre-authorization tree. Asserts no derived identity |
| `autoinit_c1_attempt5_grant.json` | the **maintainer decision permitting ONE C1 session -- attempt 5**, at the pod-fidelity closeout of 2026-09-05. A NEW artifact; the four prior grants were each consumed by one issuance and attempt 4's remains consumed. States only maintainer-owned fields: who permitted what, at `$265.0864` of the unchanged `$283.7600` cap, and the twenty things it does not authorize. Records that the `$3.5261` remaining after one full ceiling is explicitly NOT authorization for another attempt, and carries the canonical issuance ORDER whose first step is the launch-bound sweep on the pre-authorization tree. Asserts no derived identity |
| `autoinit_c1_attempt4_grant.json` | the **maintainer decision permitting ONE C1 session -- attempt 4**, at the final pre-provider review of 2026-09-05. A NEW artifact because each prior grant was consumed by one issuance. States only maintainer-owned fields: who permitted what, at `$264.3878` of the unchanged `$283.7600` cap, and the seventeen things it does not authorize. Records the canonical issuance ORDER -- grant, launch-bound sweep, readiness-record commit, issuance, authorization commit -- because a sweep run after issuance would add a second path to the lineage diff and `session_commit_gate` would refuse. Asserts no derived identity; the issuer refuses a grant that does |
| `autoinit_c1_attempt3r_grant.json` | the **maintainer decision permitting ONE Attempt-3R C1 session at the repriced `$1.09/h` secure rate**, a NEW artifact because the three prior grants were each consumed by one issuance. Records the repricing (which field the launcher prices on, and that `communityPrice` is a different product), the supersession of `955c9288` as SUPERSEDED UNLAUNCHED at `$0.00`, all three prior aborts, and the full failure semantics including `market_price_above_ceiling`. Asserts no derived identity |
...
| `c1_renderer_parity.json` | **pre-provider gate 11's evidence.** All seven pinned Hugging Face source snapshots re-opened on the dev box and all seven groups of frozen `recovery_search_v2` prompts re-rendered through the shared renderers, byte for byte: 7 PASS, 0 SKIP, 0 FAIL, 190 prompts. Written by `scripts/autoinit/renderer_parity_gate.py`, which shares `check_group_parity` with the pytest cases rather than reimplementing "identical". The guarantee moved here when those cases became skippable on hosts without the ~4 GiB of sources — a **readiness input only**, never staged to the pod and read by no C1 number. The launcher re-executes the check live at launch; this file is the human-readable copy |
| `c1_pod_environment_verification.json` | **pre-provider gate 12's evidence.** One complete pod-like sweep of the CPU test suite under the conditions a fresh pod is actually in — empty `HOME`, isolated `HF_HOME`, `HF_HUB_CACHE` beneath it, synthetic non-empty `HF_TOKEN`, gitignored artifacts hidden, the pod gate's own test selection. Records per-nodeid counts, the exact seven expected renderer-parity skips, the five leaf-transport nodeids that must PASS, and a tree-clean verdict. Binds the **executable** — the C1 harness digest and a pod test-environment digest over `tests/**/*.py` plus the setup script, the simulator and the modules whose `$HOME` assumptions caused the abort — and deliberately **not** HEAD, so preregistration and documentation commits do not invalidate an hour-long sweep while any executable edit does. Exists because attempt 3R cleared ten gates and died on this suite for `$0.3482` |
