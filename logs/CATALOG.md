...
| `autoinit_recovery_continuation_attempt7/` | continuation attempt 7 — **PHASE A COMPLETE**, `ALL_DONE`, $12.8587. All six stages, 11 probes trained and scored across seeds sa/sb/sc; result `unresolved_equivalence`, winner `None`, with `cca699c93f34` separated from the canonical control and tied with `85bde4ded2c3`. Carries `phase_a_result.json`, both rung selections, leaf retention, all 11 probe records, the attested protocol, the driver run log and the launcher/watchdog/poller journals. The launcher crashed AFTER collection on a `fetch_products` contract mismatch, mislabelling the session INCOMPLETE |
