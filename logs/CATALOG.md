...
| `autoinit_c1_bundle.json` | the local half of the C1 transport check, written by `stage_c1_bundle.py`: session commit, canonical bundle name, relay path, sha256 and bytes. `bundle_staged_gate` verifies the REMOTE object against it. Currently records the Attempt-3R bundle `aad_autoinit_26fd4cc9.bundle` (7,449,366 bytes, `473396ae3f68…`) |
