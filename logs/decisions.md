...
- **This is a NULL RESULT and stays one.** No Top-5, no `sa`/`sb`/`sc`, no cross-phase selection. It says nothing about `calib.reasoning_heavy@v2` versus `calib.domain_balanced@v1`, and it must not be reinterpreted as a winner, a tie, or an `unresolved_equivalence`. Those are outcomes of a procedure that did not run.
