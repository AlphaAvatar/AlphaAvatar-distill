...
- **Revisit when:** `analyze_e3.py` reports. If R4 fires — both arms improve FineWeb NLL but neither improves rollout — freeze-policy exploration stops and the recommendation moves to student-prefix / on-policy recovery, which is where `src/aadistill/rollout/`'s 2,075 untouched lines already point.
