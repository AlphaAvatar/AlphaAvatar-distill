...
* `experiments/` — per experiment: plans, analyses, results, history
* `maintenance/` — storage, inventories and cleanup records
* `runs/` — every run, by declared stage or unscoped, and the index
* `state/` — what is true now: current.json, current.md, ownership

### REFERENCE

* `migrations/` — how old paths map forward; the relocation provenance
* `validations/` — engineering validation evidence -- not scientific runs
