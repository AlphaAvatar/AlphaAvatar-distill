...
| limit | remaining |
| --- | --- |
| formal sessions | `$44.2133` of `$45.4425` |
| GPU engineering | `$6.0000` of `$6.0000` |
| package | `$50.2133` of `$51.4425` |
| project cap | `$269.1290` spent of `$320.0000`, leaving `$50.8710` |

**Full-ceiling sessions the FORMAL allowance funds: 2.** 3 ceilings cost `$45.4425` and the formal allowance has `$44.2133`. Dividing the PACKAGE balance instead gives 3, which is the error: the engineering allowance cannot pay for a formal probe.
...
## Where attempt 16 stopped

Attempt 16 cleared all fourteen gates, completed setup — with a **4-second** CPU
test gate where attempt 14 spent 982 seconds — and its driver failed nineteen
seconds later in stage D, `replay_parent`:

```text
KeyError: no architecture adapter registered for family 'qwen3'; registered: []
```

Registration stopped being an import side effect when the adapter registry
became something a caller builds. The Phase-A driver, the recovery-continuation
driver and six scripts picked up the explicit call; the C1 driver did not. It is
the seam that extraction left unexecuted, and it was invisible to the suite by
construction — a process-default registry is empty in a fresh process and full
in any pytest session where another test registered it.

Repaired at the driver, and covered by a preflight check that imports the driver
in a **subprocess** and resolves the adapter for the family the pinned
checkpoint declares. Mutating the fix away reproduces attempt 16's exact error.

Stage D never completed a comparison, so this is **not** a replay mismatch and
says nothing about the frozen path. Earlier the same day, attempt 14 died at the
pod CPU gate on two dev-box-only premises and attempt 15 at `$0` on a two-path
lineage diff; both are closed, and
[`budget/decisions.md`](../budget/decisions.md) carries the reasoning.
