...
- **What actually raised the duplicate-seed exception, established mechanically rather than from prose.** Only **three** `PROBE_TRAINED` markers exist, for the three genuinely new leaves; `cca699c93f34` and `85bde4ded2c3` were never trained. `probe_configs` is a plain list comprehension over the universe with no dedup, so the 8-entry universe produced 8 descriptors over 6 distinct state ids, and `selection_row` groups by state id. The driver log shows each of those two **cited twice** at stage-2 entry. So the failure came from **one searched descriptor and one imported alias restoring the same historical `sa`** — not from two paid probes. The attempt-5 record is corrected accordingly.
- **The two exact reproductions are proven three ways.** `identify_checkpoint` re-derived on the retained Phase-A directory, on the nested copy attempt 5's teardown wrote, and as recorded by the Stage-1 selection artifact — all three agree: `04fa7fe74f01ee63` and `590156f691fb10bf`. Same content-derived state id, same materialized artifact digest, same bytes.
- **The collapse rule decides on identity and nothing else.** Collapse requires the canonical state id **and** the re-derived artifact digest to agree. Same id, different digest is `IdentityCollapseError` — a content-derived id disagreeing about its own bytes is a broken identity, not a duplicate to merge. Names, id prefixes, composition descriptions, `sa` scores, behavioural ranking and correctness outcomes are **never** inputs, which is what lets the amendment be written after the collision was seen without the outcome shaping it.
- **Roles, not treatments.** Searched is primary for an initialization the search rediscovered; imported survives as an evidence alias contributing citable provenance. **No Top-5 backfill** — ranks 6/7 are not promoted to restore an artificial count of eight. The universe is **6 distinct candidates**, identity `e94f15d2e648d86a…`, amendment `df413bd99119dab7…`.
- **`POOLED_COUNTS_V2` is untouched.** Its refusal was correct and the fix is upstream: one observation per `(materialized initialization, seed)`. `observations_per_seed` drops an exact duplicate — same state, same seed, same digest, cited under two roles — and **refuses** two observations of one seed that disagree on bytes, because that is a conflict.
- **All three fresh `sa` probes reconstruct strictly**, to the same standard as the historical citations: complete, frozen `SEED_SA`, digest re-derived from retained bytes, digest matching the Stage-1 selection, `recovery_search_v2` by content and manifest, live scoring contract, attested protocol hash, poolable counts, and no conflicting duplicate observation. `fe9683e6a9c7`, `bf5ae3b6ae00`, `ab7632b00788` — none needs re-running.
- **Rung 1 was computed at `$0` by the frozen `select_rung1_survivors`, unchanged.** Six candidates all had `sa`. Survivors `fe9683e6a9c7` (0.0471) and `85bde4ded2c3` (0.0412); control auto-advances; no feasibility or catastrophic exclusions. I did not choose them and did not look before the code ran. Note `bf5ae3b6ae00` and `cca699c93f34` tie exactly at 0.0353 in ranks 3–4, below the cut.
- **What remains is small.** Only `fe9683e6a9c7` lacks `sb`; at most `fe9683e6a9c7` and the control lack `sc`. **One to three probes**, against the ten a full Phase B books. Continuation floor **$5.4784**, **ceiling $8.0691** — and the current `$23.7050` headroom already covers it, so **no cap increase is required**. Implied cumulative cap from `$260.0550` spend: **$268.1241**.
- **D1** — the Stage-1 selection writer omitted `arch_signature`, which `verify_transferred_leaf` reads off the record, so all five transfers reported NOT MATCHED on a `KeyError` while the bytes were correct. The writer now emits it and `weights_digest`, and a test derives the required set **from the consumer's source** rather than from a list I maintain. Attempt 5's frozen artifact is **not** edited; a separate retention-verification record supplies the identities, re-derived independently.
- **D2** — the teardown reported clean while its own secured gate said "deleting the pod now would destroy exactly the science it paid for". It now records an explicit `PRODUCTS_UNSECURED / ARTIFACT_LOSS_RISK` terminal condition, carries it into the teardown reason, and returns `done and secured_ok`. Deliberately **not** a block: an unbounded hold on the most expensive pod in the project is a worse failure than a recorded risk.
- **D3** — retention is digest-aware. An already-retained checkpoint whose re-derived digest matches is **deduplicated with zero bytes transferred**; same id with different bytes is refused and never `scp`-ed into. That is what turns a rediscovery from a 1.2 GiB nested duplicate into a no-op.
- **Cleanup: 2273 MiB reclaimed.** Both nested copies were proven byte-identical to their canonical parents before removal; the canonical Phase-A files were never touched and still carry their 2026-08-21 timestamps; and the historical citations still reconstruct 11/11 with 8 admitted afterwards. Derived duplicate cleanup, not artifact retirement.
- **The preregistration is not rewritten.** It still reads "8 distinct at sa". The amendment records that the assumption was falsified by a reproducible search and what replaces it.
- **Identity movement:** executable source `3ad6a6e3edf1…` → **`3d3b5d0717723cd3…`** over **60** files at set version **6** (`identity_collapse.py` joined — it decides which candidates are distinct, which is the behavioural universe itself). Preregistration re-frozen **`8d5aa123423b353c…`**. Attempt-5 Stage-1 result, Top-5, calibration identities, P=2 science, ranking policy, feasibility floor, equivalence interval, seeds, no-fourth-seed and recovery scoring all unmoved.
- **Revisit when:** the maintainer gives GO for one behavioural-continuation session.

## 2026-08-29 — Attempt 5: the search finished, and the candidate universe collided with itself

- **Context:** attempt 5 launched at `8d6ba98e` under `autoinit.phase_b.20260828T070326Z`. Pod `37aah10zvqk4lo`, 725.7 min, **$11.9700**, deleted and provider-confirmed by the launcher and by an independent `runpodctl pod list`. Outcome **`EXECUTION_INCOMPLETE / NO_SCIENTIFIC_RESULT`** for the calibration question.
- **The furthest any attempt has reached, and the first with a session-emitted scientific artifact.** Stage 0 passed in 3.0 min. **Stage 1 completed the joint P=2 search in 464.1 min of its 987.6 min allowance and wrote `search_result.json` with an authoritative Top-5** — the attempt-4 shadowing fix held, in production. Stage 2 then trained and scored **three new `sa` probes** before failing.
- **The failure is a scientific finding wearing a stack trace.** `ValueError: duplicate seeds in [20260726, 20260726]` from `POOLED_COUNTS_V2.pool`. Two of the five searched leaves — `cca699c93f34` and `85bde4ded2c3` — are **byte-identical to two retained Phase-A finalists**. Verified from two directions: `identify_checkpoint` re-derived on the retained Phase-A directory and on the Phase-B copy that landed inside it give the same artifact digest, `04fa7fe74f01ee63` and `590156f691fb10bf`.
- **Why that happened is the reproducibility contract working.** State ids are content-derived. Phase B searched a strictly larger space at P=2, rediscovered the same two compositions from the same root under `calib.domain_balanced@v1`, and produced the same bytes. An independent session reconstructed a prior session's checkpoints exactly — which is the strongest determinism evidence this project has.
- **And it is exactly why Stage 2 could not proceed.** The preregistered universe is "5 searched + 2 imported + 1 control = 8" and assumes those are **distinct**. In fact there were **6 distinct candidates**, and two ids each carried a cited historical `sa` probe (seed 20260726) *and* a freshly run one at the same seed. Pooling refused, correctly — counting one seed twice would inflate the evidence. **The guard did its job; the frozen rule has no way to say "a searched leaf turned out to BE an imported candidate".** Changing that is a change to the preregistered procedure and is a reviewer decision.
- **Three mechanical defects, none of which cost the science this time.**
  - **D1** — all five leaf transfers reported `rc=1 digest=NOT MATCHED`. The cause is mine: `verify_transferred_leaf` requires `record["arch_signature"]` and the Stage-1 selection record I added last pass never provides it, so it raised `KeyError` and was caught into `rc=1`. **The bytes are fine** — all five landed complete and all five digests re-derive correctly. A writer/consumer contract gap, the same class as the `harness_source_*` aliases and the `SESSION_KIND` branch, and the third time I have shipped one.
  - **D2** — `products_secured` returned False with the message "deleting the pod now would destroy exactly the science it paid for", and teardown proceeded anyway: `teardown gate: allowed=True`. The outer gate does not consult the secured verdict. It cost nothing here only because D1 was a false negative; had the transfer genuinely failed, a completed search would have been deleted.
  - **D3** — `fetch_selected_leaves` writes to `ckpt_store/phase_a/<state_id>`, which for the two reproduced leaves already existed. `scp -r` nested rather than overwriting. **No damage**: the Phase-A files keep their 2026-08-21 timestamps and the historical-reuse verifier still passes 11/11 with 8 admitted. The cost is 2 × 1.2 GiB of nested duplicates in the retained store, on a filesystem at 96%.
- **The Stage-1 durability mechanism fired and was useful.** "stage-1 selection present with no retention report: securing 5 selected leaves from the search workdir" — Stage 2 failed before writing a retention report, exactly the case it was built for, and it is why all five checkpoints are on the dev box now.
- **What is NOT reported, and why.** No `sb`/`sc` progression, no cross-phase selection, and **no `winner=None`** — that is a frozen outcome of a procedure that ran, and this one did not. The three new `sa` scores sit beside three citations on an incomplete rung with no admission step; they are not a ranking and are recorded with that warning attached. The Stage-1 Top-5 is a **search** ranking on `state_eval@v1`, not the behavioural comparison Phase B exists to make.
- **Money:** `$260.0550` of `$283.76`, leaving `$23.7050` — below the `$35.6660` ceiling, so a sixth attempt is blocked without a new cap decision. Phase-B attempts now total **$30.0200**.
- **Revisit when:** the reviewer rules on the reproduced-candidate collision, on the three defects, and the maintainer on the cap.

## 2026-08-28 — The shadowing fix, and a completed Stage-1 that can survive the code after it
...
- **The injected-failure test is the one attempt 4 asked for:** search completes, ranking completes, selection is committed, the next bookkeeping step raises — and the selection is still on disk and still verifies; then the collector identifies, fetches and secures the five from **their own recorded checkpoint paths**, not from a stage-2 staging directory a stage-1 failure never created.
- **Eight mutations killed**, including reverting the shadowing, dropping `generated_utc` from the exclusion list, a collector that ignores the selection, a secured-gate that ignores it, a `load()` that skips its own hash check, and a fetcher that uses the staging directory instead of the recorded path.
- **A record correction I owed.** I had written that the journal held "5 searched + 2 composite + 2 imported finalists = 9 complete leaves". Mechanically it holds **9 BeamSearch leaves, every record `provenance="search"`** — 2 composite and 7 decomposed — and **no imported finalists at all**: `make_retained_state` builds them outside `BeamSearch` and nothing appends them to the store, so attempt 4's measurements of them are gone. Two decomposed leaves carry the Phase-A finalists' state ids because ids are content-derived and the same composition was rediscovered from the same root. Reading that as the finalists appearing in the journal is what produced the error.
- **The journal stays audit evidence and does not become a result.** Reviewer decision, and the right one: without the Top-5 checkpoint bytes it cannot satisfy the restore contract, which requires a re-derived artifact identity from actual bytes. No post-hoc Top-5, no `sa` from journal-only ids, no one-off replay path.
- **Pricing verified unmoved**, which the repair had no business changing: floor `$16.4555`, ceiling `$35.6660`, search hard `$16.2960`, all byte-identical on regeneration.
- **Identity movement:** executable source `d232e5c7…` → **`3ad6a6e3edf1…`** over **59** files at set version **5** (`stage1_selection.py` joined: written inside the paid search, read by the failed-run collector). Preregistration re-frozen **`53f1347f6865…`**. Science plan, session plan, both calibration spec and content identities, candidate universe, Top-5 rule, finalists/control, `sa/sb/sc`, feasibility floor, equivalence interval and seeds all verified unchanged.
- **Revisit when:** the maintainer rules on the cumulative cap. `$248.0850 + $35.6660 = $283.7510`; the reviewer's recommendation is `$283.76`.

## 2026-08-28 — Attempt 4: the search finished, the sentence describing it did not

- **Context:** attempt 4 launched at `27a366e8` under `autoinit.phase_b.20260827T102821Z`. Pod `zjwpsurs2dyvw8`, 495.2 min, **$8.1700**. Deleted after the teardown gate; provider-confirmed by the launcher and by an independent `runpodctl pod list`. Outcome **`EXECUTION_INCOMPLETE / NO_SCIENTIFIC_RESULT`**.
- **It got further than any attempt before it.** Stage 0 passed in 2.2 min. Stage 1 ran 478.5 min of its 987.6 min allowance, **completed the joint P=2 search**, measured the canonical control, measured both imported Phase-A finalists inside the search as the design requires, and computed `result.top_n(PARETO_V1, ...)`. Then it raised while assembling the summary dict.
- **The defect is one line, and it is Phase-B-only.** `scripts/autoinit/phase_a_search.py` binds the `SearchConfig` to a local named `config`. The `for entry in retained_candidates:` loop rebinds it: `config = AutoConfig.from_pretrained(str(directory))`. Twenty lines later the summary reads `config.run_id` and gets a `Qwen3Config`. **That loop body executes only when retained candidates are passed — Phase A passes none.** So every Phase-A run, every continuation and every zero-cost test walked straight past it, and the first execution was eight hours into a paid session.
- **Same class as the last three, and I should say so plainly.** `SESSION_KIND=phase_b` had no dispatch branch; the artifact specs collected the Phase-A journal path; now a Phase-B-only loop shadows a name. Each was reachable at `$0`, each was invisible because the Phase-B path is the one nothing else executes, and each cost a paid session to find. The pattern is not "Phase B is unlucky" — it is that **Phase-B-only code has no cheap execution path**, and adding one is the repair that would have prevented all three.
- **Not fixed here.** The instruction after a new material blocker is to stop and report, and the fix needs more than a rename: a test that actually executes the retained-candidates path, which is the thing that has never run outside a pod.
- **Both previous repairs are confirmed on hardware, which is the real result of this run.** The search journal came home — `states.jsonl`, 53 MB, 155 records over 82 states — from the Phase-B path the corrected specs collect; attempt 3 lost exactly this. And P1 ran in `partial` mode in all 12 causal-depth expansions ("caching 52/67 items (78%, 13.2 GiB), recomputing the rest"). **The same 12 expansions cost 287.3 min against attempt 3's 388.2 — 101 min less, a 26% reduction**, mean 23.9 min against 32.3. Modelled saving was 41.8% of forward work; 26% end-to-end is the same number diluted by the non-forward share, and it is measured rather than assumed.
- **The telemetry's first reading accounts for 98.7% of Stage 1 and immediately falsifies part of the cost model.** Operator 320.2 min (67.8%); **state evaluation 147.2 min (31.2%)**; materialize, identify, reload and validate together 4.8 min (1.0%). State evaluation is priced in the model as pure FLOPs at roughly 0.04–0.08 min per child and actually costs about 1.8 min — underpriced by more than an order of magnitude — while the materialization overhead I added is *over*priced by roughly 10x. The hard bound stays conservative overall because the two errors point in opposite directions, which is exactly the accident the last pass warned about. **Recorded and not acted on: pricing research is closed by instruction.** One run, 82 expansions, one beam position — a sample, not a law.
- **Stage 1's scientific output exists and is retained, and I am not reporting it as a result.** The journal holds 9 complete leaves — 5 searched, 2 composite, and the 2 imported finalists. The ranking the run computed was never written to an artifact, and it is deterministically reconstructible from the journal under the frozen policy. Reconstructing it post hoc and presenting it as the session's Top-5 is precisely the move this project's rules forbid, so it is not in the record as one. **Whether a retry may cite this search rather than repeat eight hours of it is a reviewer decision.**
- **Money:** `$248.0850` spent against `$275.59`, leaving `$27.5050` — below the `$35.6660` session ceiling, so a fifth attempt is arithmetically blocked. Phase-B attempts total **$18.0500** with no scientific result. Attempts 1–4 remain historical setup/execution-incomplete evidence.
- **Nothing scientific moved:** no stage past 1 ran, so the search science, candidate set, thresholds, seeds, both calibration identities and the permanent controls are untouched. No checkpoints were retained — Stage 1 never reached the retention step.
- **Revisit when:** the reviewer rules on the fix, on citing attempt 4's search, and the maintainer on the cap.

## 2026-08-27 — Ceiling and cap approved; the poll lifetime is derived, not inherited

- **Maintainer decision:** Phase-B session hard ceiling **$35.6660** APPROVED, cumulative cap **$275.59** APPROVED, spend unchanged at **$239.9150**. P1 partial causal-DEPTH caching, P2 two-profile statistics caching and the corrected pricing model are frozen. Attempt 4 authorized after the poll-lifetime repair and all existing gates.
- **The repair: Phase B was polling for 22 h against a 32 h session.** `--poll-limit-min` defaulted to Phase A's historical **1320 min**, and the corrected Phase-B plan terminates at **1925.87 min (32.10 h)**. The launcher would have stopped polling **606 min — just over ten hours — before the session reached its own operational hard bound**, and a launcher that stops polling exits while the pod does not. That is this project's most expensive failure mode, and `--terminate-after` has never once been observed to stop a pod.
- **Derived, not renumbered.** Replacing 1320 with 2100 would have been right today and wrong the next time the envelope moved — which is exactly how 1320 became wrong. `phase_b_poll_limit_minutes` reads the priced plan's `hard_terminate_minutes` and adds slack built from knobs that already exist: one `--poll-seconds` interval of granularity, `--ckpt-fetch-limit-min` for the one post-terminal step that moves real bytes, and **10 min** of teardown measured from attempt 3 (marker at 02:40:27, pod deleted and provider-confirmed at 02:43:15 — 2.8 min, with no products to transfer). **1967.87 min, 42 min of slack.**
- **Phase A's 1320 is untouched**, and a test pins it: several completed sessions ran under it and rewriting it would retroactively change what they mean.
- **The default is `None` in the parser and resolved in `main()`**, so no constant can sit in the parser going stale, and `poll_lifetime_gate` refuses before a pod exists if the resolved value does not clear the hard bound with room to collect and tear down. The gate matters because `--poll-limit-min` is still a flag a caller can set to anything.
- **Eight mutations; seven killed and one is equivalent.** Weakening the gate's `limit <= hard` to `limit < hard` changes only the refusal *message*, because the slack check below it already refuses equality. That guard is kept for the message a reader needs at 3 a.m., and the code now says so rather than implying it is load-bearing. Reporting it as a kill would have been wrong.
- **Nothing scientific or priced moved.** The pricing record is byte-identical apart from its timestamp: search hard `$16.2960`, floor `$16.4555`, ceiling `$35.6660`. Science plan `02be33b9…`, session plan `23f23649…`, both calibration spec and content identities, the reasoning-heavy items identity, the candidate universe, Top-5 rule, imported finalists and control, `sa/sb/sc`, thresholds and seeds all unchanged. The session plan hash does not carry operational defaults, so the poll lifetime does not touch it.
- **Identity movement, as expected:** `autoinit_phase_b_launch.py` is in the executable set, so the digest moved to `d232e5c75ec7…` over the same 58 files at set version 4, and the preregistration re-froze to `1d3d8079b906…` against it.
- **Revisit when:** attempt 4 reaches a terminal outcome. Nothing follows it without a new maintainer decision.

## 2026-08-27 — Pricing closure: composite, shared statistics, and the non-FLOP path

- **Context:** a second `$0` pricing pass. No pod, no canary, no grant, no launch. The P1/P2 optimizations are untouched as instructed — `BUDGET_FRACTION` 0.66, admission by frozen mixture order, one stats entry per active profile, clear at the parent boundary, parent artifact digest in the key.
- **Composite was counted and not costed.** `composite.stage1_sandwich_v0` reached `branching_estimate` only, so its two P=2 leaves appeared in `states_materialized` and `leaves_max` while contributing nothing to time or money. Each one consumes activation statistics, transforms, materializes, reloads, validates and is evaluated. It is now a level-0 expansion in the cost accumulation, branching over profiles because it consumes calibration.
- **Statistics were charged per operator, not per collection.** `StatsCache` shares one pass per `(parent, profile)` across `composite`, `ffn` and `width`; the model billed each of them separately. It now charges `n_profiles` collections per parent — **except at the root**, where `BeamSearch._stats_key` returns `None` because the root's identity is a published revision rather than an artifact the search computed, so sharing has nothing to key on. Level 0 pays **six** collections; a level-1 parent pays two. Level 0 is the widest level in the search, and pricing it as though it shared would have understated exactly the operators that make it wide.
- **The two errors were partly cancelling, which is why they had to be fixed together.** The hard bound moved only `994.4 -> 987.6 min` while its composition changed materially. A test asserting the total alone would have passed against both defects; the mutation `M4` applies both at once and is killed by the two structural tests rather than by any total.
- **The non-FLOP path is now explicit.** `save -> hash/identify -> canonical reload -> validate` is I/O, hashing and Python. `L40S_MEASURED.effective_tflops` was measured from 260 subset evaluations — pure forward compute — so using it to cover checkpoint I/O would be reading a number off an instrument never pointed at it. Charged per materialized child as `3 x checkpoint bytes` at a deliberately pessimistic **400 MB/s** plus **10 s** fixed: 1.08 min for a 6.79 GiB root child, 0.32 min for a 1.11 GiB leaf. The search contract is unchanged — every child still saves, hashes, reloads, validates and is measured.
- **A mutation survived the first pass and exposed a real hole.** Replacing the per-parent maxima with the level mean inside `conservative_hard_seconds` killed nothing, because my average-vs-bound test built its "average" from the `high` world and compared it against `seconds_hard` from the `hard` world — two different models, so the substitution was invisible. The estimator now reports `mean_parent_seconds` and `chosen_mean_seconds` per level and the test compares them **within one world**. It also had to learn a real distinction: where the beam can hold every distinct geometry (level 2, six of six) the bound legitimately equals `parents_max x mean`; the strict inequality is asserted only where the beam is narrower than the geometry.
- **Replay against attempt 3 holds.** Its 12 causal-depth invocations reconstruct exactly — 2 at the root, then 10 at level 1 (five of the eight level-0 children had not applied DEPTH, times two profiles), dying at a level-1 FFN. The hard model bounds levels 0+1 at **749.4 min** against the **544.7 min** the run spent inside them without finishing, and level 0 alone at **130.6 min** against the **76.1 min** its two root invocations took. Per expansion, recomputed spans 8.7-45.8 min against 28.5-38.4 observed.
- **Final figures:** search planning **$5.2705-$9.5066** (5.324-9.603 h); search hard **$16.2960 / 16.461 h / 987.6348 min**; Stage-1 deadline **987.6348 min**; session floor **$16.4555** (was `$13.0800` — the low scenario now includes composite, the root's unshared statistics and the materialization overhead, and it remains a floor rather than an expectation); session hard ceiling **$35.6660**; implied cumulative cap from `$239.9150` spent: **$275.5810**.
- **The frozen `$26.8049` is still unchanged and still fail-closed.** `plan_session` refuses the session under it by `$5.09` against a plan hard of `$31.7769`. Attempt 4 cannot launch until the maintainer rules.
- **One backlog correction, mine:** cross-profile causal-depth score sharing was recorded as scientifically unsafe. That overstated it. The *aggregated* scores genuinely differ because each mixture aggregates over its own items, but the item-level work for a given `(parent, skip-set, item_id)` is the same forward and could be computed once and aggregated per mixture. It is **deferred on complexity**, not rejected on science.
- **Identity movement:** executable source re-derived to `749241f9300c…` over the same 58 files at set version 4 (`cost.py` and `price_phase_b.py` changed); preregistration re-frozen `d3343e8656…`. Science plan, session plan, both calibration spec and content identities, the candidate universe, thresholds, seeds and the behavioural procedure all verified unmoved.
- **Revisit when:** the maintainer rules on the ceiling and the cap.
...
- **The equivalence proof is the load-bearing part**, not the speed: cached, partial and recomputed produce identical removal order, kept layers, per-round candidate score tables and operator metrics — asserted exactly, not approximately, because the reference is the same deterministic no-grad forward either way.
- **P2 — the statistics cache holds one entry per active profile.** `_candidate_expansions` yields ffn(db), ffn(rh), width(db), width(rh); a single entry made all four miss, costing **four** passes where two would do. The reuse boundary is untouched — the key still leads with the parent's artifact digest — and the search now clears the cache at each parent boundary so residency is bounded by the parent being expanded rather than by eviction order. At P=1 the behaviour is bit-for-bit what it was.
- **A mutation that survived, and what it taught.** Sorting admission by item size instead of mixture order killed nothing, because every item in the shared toy fixture is 24 tokens long and the sort is a stable no-op. The test now builds items of differing lengths and a budget under which the two rules genuinely disagree. A fixture that cannot distinguish two implementations is not a test of either.
- **The cost model was invalidated by hardware, and is re-derived.** The old range assumed the intact reference was paid for **once**; attempt 3 paid for it **per candidate**. `greedy_depth_flops` now prices all three modes, and the estimate reports low=`cached`, high=`partial`, hard=`recomputed`. The hard figure assumes **no** caching, because the bounded cache guarantees a fraction and a ceiling may not rest on one.
- **`children_max x mean node cost` is gone as the ceiling.** An average is not a bound: node cost at one level varies by more than 2x with the parent's geometry, and some operators cost ~0 (a positional heuristic takes no measurement), so a beam that retains causal-depth parents costs more than `children_max` averages. `conservative_hard_seconds` takes, per level, the `parents_max` most expensive parent **geometries**, each costed over its own expansions with no-calibration operators counted once. Structural, and it uses the frozen beam width rather than a statistic.
- **Checked against work that has already happened.** Recomputed mode spans 8.7-45.8 min per expansion against 28.5-38.4 min observed, so the observed band lies inside it. Cached mode's *most* expensive expansion is 21.7 min — below the **cheapest** invocation actually observed — which is precisely why the old range was wrong. The old 7.51 h ceiling is below the 9.08 h attempt 3 spent without finishing.
- **New figures, all mechanically derived:** search planning range **$1.8950-$8.0213** (1.91-8.10 h); search hard bound **16.573 h / 994.395 min / $16.4075**; Stage-1 deadline **994.3950 min**, which lands exactly on the hard bound because the two inherited soft-stop reserves are **subsumed** by it and are subtracted from the base rather than added on top; session floor **$13.0800** unchanged; session hard ceiling **$35.7775**; plan hard terminate $31.8995. Implied cumulative cap from $239.9150 spent: **$275.6925**.
- **The frozen $26.8049 was NOT changed, and that is now a fail-closed blocker.** The reviewer's instruction listed it as an identity that must not move, while also asking for a new per-session ceiling; I derived the new figure and left the constant alone. `plan_session` therefore **refuses** the session under $26.8049 by $5.09, with the number in the error. Attempt 4 cannot launch until the maintainer approves the new ceiling and cap — which is the right shape for that decision.
- **Identity movement:** executable source `45f6bb8c…` -> re-derived over **58** files at set version **4**; `scripts/autoinit/price_phase_b.py` joined the set because the launcher now imports it to derive the Stage-1 deadline, the same judgement that admitted the reuse verifier at version 3. The preregistration was re-frozen against it. Science plan, session plan, both calibration spec and content identities, the reasoning-heavy items identity, the candidate universe, Top-5 admission, imported finalists and control, `sa/sb/sc`, the feasibility floor, the equivalence interval and the `$13.0800` floor all verified unmoved.
- **Attempt 3's semantics corrected everywhere:** `EXECUTION_INCOMPLETE / NO_SCIENTIFIC_RESULT`, not "null result". Phase A's `unresolved_equivalence` is a *completed* outcome — the procedure ran to its end and the candidates were indistinguishable inside the frozen interval. Attempt 3 is an execution failure. Conflating them would let a deadline overrun be cited as evidence about the calibration hypothesis.
- **Revisit when:** the maintainer rules on the ceiling and the cap. Nothing else is blocked.

## 2026-08-27 — Phase B ran, Stage 0 passed, and the search ran out of time

- **Context:** attempt 3 launched at `4a25699a` under `autoinit.phase_b.20260826T164611Z`. Pod `wkausr939ts7vv`, 575.9 min, **$9.5000**. The first Phase-B attempt to enter the science. Pod deleted after the teardown gate passed; provider confirms gone by the launcher *and* by an independent `runpodctl pod list`.
- **Both prior blockers are closed.** The pod's test suite passed (`TESTS_OK:399s`) and the new `phase_b` dispatch branch verified the authorization (`AUTHORIZATION_OK`). Attempts 1 and 2 died at exactly those two steps.
- **Stage 0 PASSED**, 3.15 min after driver start: `interval 0.011695, floor 0.3000, plan 02be33b9a7a8…`, the executable digest and both calibration spec/content/items identities rebound on the pod, and 8 historical probes imported with 3 skipped for not being in the candidate set. Comparability held, so **all historical reuse was preserved** and the ten-probe budget stood; the rejected 14-probe no-reuse path was never reachable.
- **Stage 1 FAILED on its deadline, and the guard behaved exactly as designed:** `SearchDeadlineExceeded: search deadline exceeded after 544.7 min (budget 544.0 min) at before ffn.activation_importance_v0 on 85421eab09ee`. It stopped rather than continuing to the `$23.72` cost backstop. The 0.7 min overrun is the granularity of the check, which fires *before* an expansion rather than during one.
- **This is `EXECUTION_INCOMPLETE / NO_SCIENTIFIC_RESULT`, not a scientific null.** Phase A's `unresolved_equivalence` is a *completed* outcome — the procedure ran to its end and the candidates were behaviourally indistinguishable inside the frozen interval. Attempt 3 is an execution failure: the procedure did not run, so there is nothing to interpret. No Top-5, no `sa`/`sb`/`sc`, no cross-phase selection. It says nothing about `calib.reasoning_heavy@v2` versus `calib.domain_balanced@v1`, and it must not be reinterpreted as a winner, a tie, or an `unresolved_equivalence`. Those are outcomes of a procedure that did not run.
- **The cost model was wrong at the top of its own range.** The P=2 search was priced at 1.91–7.51 h; it ran 9.08 h and had not finished. Reconstructed from the driver log: `depth.causal_kl_greedy_v1` ran **12 expansions**, **388.2 min total = 71.3% of the search**, mean **32.3 min** each (range 28.5–38.4), 260 layer candidates and 260 teacher evaluations per expansion. The remaining 156.5 min covered materialization, reload/validate round-trips, `state_eval` measurement, statistics, and the operators that log no per-candidate line. **P=2 doubles expansions rather than sharing them**, and DEPTH is where that doubling lands.
- **What is NOT known, stated plainly:** how many search *states* completed, and how close the beam was to five admissible leaves. Only DEPTH logs per-candidate progress, so 12 is a firm count of DEPTH expansions and not of total expansions.
- **The evidence gap, and it is the same class again.** `configs/autoinit/phase_a_artifacts_failed.json` collects `search_journal` at `autoinit/phase_a_search/states.jsonl`; the Phase-B driver writes `artifacts/autoinit/phase_b_search/states.jsonl`. One directory name apart. The entry is `required:false, min_matches:0`, so the manifest reported `missing:0` and the teardown gate passed while the single most useful artifact for a *deadline* failure was left on a pod that is now deleted. The Phase-B driver overrode the workdir; the artifact spec, a different file, still names the Phase-A path. "Ask who else loads it" applies to output paths and their collectors, not only to loaders. **Recorded, not fixed** — the instruction after a terminal outcome is to stop.
- **A correction I made before reporting:** my first reconstruction of the DEPTH cost summed the per-line `min` field and produced 51,337 min, or 9,425% of a 544.7 min search. The field is cumulative within an invocation, not additive. The corrected figures are the ones above; an arithmetic result that impossible is a signal to re-read the source, not to publish.
- **Money:** `$239.9150` spent against a `$257.22` cap leaves `$17.3050` — below the `$26.8049` ceiling. A fourth attempt is arithmetically blocked and would need a repriced allowance regardless. Phase-B attempts total `$9.8800`.
- **Nothing scientific moved and no checkpoints were retained:** the search produced no admissible leaves, so there were no Phase-B Top-5 bytes to transfer, and the dev-box artifact store is unchanged. The permanent Stage-3 controls were untouched.
- **Revisit when:** the reviewer decides how to reprice the P=2 search allowance from the measured 32.3 min DEPTH figure, and the maintainer decides on the cap. **STOPPED here as instructed** — no generalization, DEPTH incumbent work, operator research, canonical Stage-1 NLL or Stage 2/3 recovery has been started.

## 2026-08-27 — Option A: the dispatcher gets a Phase-B branch, and a gate that proves the mapping
...
- **Deviation from the instructed step order, deliberately and in the safe direction:** the reviewer listed commit/push *before* recompute and re-freeze. I ran the full suite and verifier first, then recomputed, re-froze, and committed once. Committing between those steps would have published a tree with two knowingly-red preregistration-binding tests, and a red tree at a commit is exactly what attempt 1 taught. Nothing was frozen over an unverified tree.
- **Money:** `$230.4150` spent, `$257.22` cap, `$26.8050` headroom against the unmoved `$26.8049` ceiling. Attempts 1 and 2 are **setup failures only** and are excluded from the Phase-B scientific evidence set.
- **Revisit when:** attempt 3 reaches a terminal Phase-B outcome. Nothing follows it without a new maintainer decision.

## 2026-08-27 — Attempt 2: the test gate passed, and the authorization gate did not

- **Context:** the Phase-B retry launched at `13d68a40` under `autoinit.phase_b.20260826T101431Z`. Pod `xgca8kkv4pm68n`, 13.9 min, **$0.2300**, **no scientific stage entered**, pod deleted and confirmed gone by the launcher and by an independent `runpodctl pod list`.
- **The repair worked.** The pod's test suite passed — **2207 passed, 84 skipped in 338.68 s** — where attempt 1 failed three tests. The strict historical-reuse reconstruction no longer runs on a pod, and it ran instead as a pre-provider gate on the dev box: *8 citations re-derived from retained bytes; evidence set `a5115816861a…` matches the record*.
- **The new blocker is one step later, and it is the same class as the last two.** `autoinit_preflight_setup.sh` dispatches the authorization check on `SESSION_KIND` and has exactly three branches — `phase_a`, `recovery_continuation`, and an `else` that loads the narrow `SpendAuthorization`. Phase B declares `SESSION_KIND=phase_b`, which matches nothing, so a `PhaseBAuthorization` artifact was loaded by `SpendAuthorization.load`, which reads `raw["preflight_plan_hash"]`. `KeyError`, `AUTHORIZATION_MISMATCH`, `SETUP_RC=98`.
- **Third collision between the Phase-B authorization type and shared machinery.** First the `harness_source_*` names the prechecks require; then the same names on the artifact; now the loader the setup script picks. Each time the type was designed for what Phase B needed to *say*, and the shared consumer was not enumerated. "Ask who else loads it" is already a recorded lesson in this project and I did not apply it to the bash layer.
- **Why no test caught it:** the dispatch is in a shell script that only executes on a pod, and nothing asserts that every `SESSION_KIND` a launcher declares has a branch. That is a purely static, `$0` check — `grep` the launchers for `SESSION_KIND`, `grep` the script for its branches, compare. It would have caught this before attempt 1, let alone attempt 2.
- **I did not fix it, and the reason is a genuine conflict rather than caution.** The standing instruction is "do not modify the shared runner/prechecks; adapt the Phase-B authorization to the already-established shared contract." Adapting the artifact means emitting `preflight_plan_hash` **and** `expected_usd` — and `expected_usd` is the field the pricing review deliberately removed, because no expected-value assumption over survivor identity or tie-break probability is defined anywhere. It would also silently bind Phase B to **Phase A's** harness file list, because `SpendAuthorization.load` falls back to `HARNESS_SOURCE_FILES_V1` when `harness_source_files` is absent: a check that passes while meaning the wrong thing. Recommendation is a fourth `phase_b` branch mirroring `recovery_continuation`, which is the pattern the script already uses and cannot affect another session kind.
- **Money:** `$230.4150` spent against a `$256.99` cap leaves `$26.5750` — **below** the `$26.8049` session ceiling. A third attempt is arithmetically blocked without a new maintainer decision. Two Phase-B setup attempts have cost **$0.3800** and produced no scientific evidence.
- **Nothing scientific moved.** No stage ran, so the search science, candidate set, thresholds, seeds, reuse rules and both calibration identities are untouched.
- **Revisit when:** the reviewer decides between the two options, and the maintainer decides on the cap.

## 2026-08-27 — The pod stopped being asked for bytes only the dev box has

- **Context:** Phase-B attempt 1 aborted in the pod's setup test gate after 8.8 min for **$0.1500**, with **no scientific stage entered**. Reviewer decision: keep the strict reconstruction strict, move it to the correct execution boundary, and retry. Maintainer raised the cumulative cap **$256.84 → $256.99** to restore the envelope attempt 1 consumed; the **$26.8049** session ceiling and **$13.0800** floor are unchanged.
...
- **Identity movement, expected and bounded.** `scripts/autoinit/verify_historical_probe_reuse.py` joined `PHASE_B_EXECUTABLE_SOURCE_FILES_V1` (set version **3**, 57 files) because it stopped being provenance the moment the launcher imported it to decide whether a pod is created. Executable source `5a84c040… → 686d43aa…`; preregistration re-frozen `c78161aa… → 3e466574…`. A field-by-field diff of the preregistration shows **only** those two identities and the file list moved: science plan, session plan, both calibration spec and content identities, candidate set, exclusions, thresholds, seeds, floor and ceiling are byte-identical.
- **Revisit when:** the retry reaches a terminal Phase-B outcome. Nothing follows it without a new maintainer decision.

## 2026-08-27 — Storage cleared, authorization issued, launch STOPPED at the first precheck

- **Context:** the maintainer raised the cap to $256.84 and the reviewer authorized one Phase-B execution. The storage-resolution pass succeeded, the authorization was issued, and the launch was **stopped** by a defect found at the final gate. **No pod was created, no paid work was performed, spend is unchanged at $230.0350.**
- **Storage: option 1 was sufficient; nothing was moved or retired.** 6.2387 GiB reclaimed from rebuildable caches in two passes — first `~/.cache/huggingface/{datasets,xet}`, `~/.cache/pre-commit` and `~/.cache/pip` (1.5000 GiB), then 4.7387 GiB of pytest scratch. **Zero checkpoints and zero scientific artifacts deleted.** `relay_retired/`, the sibling AlphaAvatar assets, the permanent controls and the measured 6 GiB margin were untouched.
- **The gate re-failed twenty minutes after passing, and that is the finding.** The full suite run between the two checks had written 4.8 GB into `/tmp`, which is on the same single filesystem as the checkpoint store. Free space measured before a suite run does not describe the state after one — which is precisely the phenomenon the 6 GiB margin was derived from, observed live. The launcher re-runs the capacity gate itself before creating a pod; that is the protection that matters.
- **I excluded `~/.local/share/uv` after checking:** `pyvenv.cfg` points the repo `.venv` interpreter at the CPython inside it, so deleting it would have broken the Python that runs every gate.
- **The issuer's first artifact failed its own tamper check**, and correctly. `as_dict()` computes `authorization_sha256`, and the issuer added five more keys afterwards; `load()` recomputes over everything but the hash, so a key added later is indistinguishable from an edit. The invalid file was deleted rather than patched — an authorization that cannot verify itself is not an authorization — and the fields now join the payload before it is hashed.
- **THE STOP: `PhaseBAuthorization` does not speak the session machinery's contract.** `session_prechecks` and `session_runner` require `harness_source_files`, `harness_source_digest` and `require_harness`. When I designed the Phase-B grant type I renamed all three to `source_files`, `source_digest` and `require_source` for clarity, and never checked what the **shared** machinery consumes. The first precheck therefore raises `AttributeError`, and the commit-lineage gate — the one that caught continuation attempt 5's stale binding at $0.1369 — never runs for Phase B at all.
- **This is the failure my own record already names.** "Inherited contracts, not needs": subclassing or reusing proven machinery means satisfying what it REQUIRES, and reasoning from what my session wanted has cost this project two pods. I reasoned from what read well in `phase_b.py`.
- **It fails before a pod, so no money was at risk** — but it cannot be worked around. The minimal repair is to give `PhaseBAuthorization` the three required names as aliases over its `source_*` fields, one source of truth. That edits `src/aadistill/autoinit/phase_b.py`, which is INSIDE the executable-source set, so `cad571c8…` moves, `ba572bb7…` must be re-frozen, and the authorization must be re-issued.
- **I did not make that edit.** The reviewer's instruction was to stop and report rather than compensate if a frozen identity would move, and to not reopen Phase-B readiness. Silently patching frozen source and re-issuing would do both.
- **The issued authorization is therefore unusable.** `autoinit.phase_b.20260825T201335Z`, `authorization_sha cc0f69cd`, bound to commit `9bf26c55` and digest `cad571c8`. It is valid in itself and recorded as **spent and unconsumable**; it must be re-issued against the repaired commit.
- **Also noted, unrelated:** the box clock reads 2026-08-25 while the session date is later, so `granted_utc` carries the machine's wall clock. The issuer forbids back-dating, so it was left as measured rather than adjusted.
- **Revisit when:** the reviewer authorizes the naming repair.

## 2026-08-27 — Phase B is approved and NOT launched: one pre-provider gate fails

- **Context:** the maintainer raised the cumulative cap to **$256.84** and the reviewer gave GO on Phase-B science, implementation and one paid execution bounded by the frozen **$26.8049** single-session ceiling. Steps 1–3 of the launch sequence are complete and pushed (`42e173a`). **No pod was created, no authorization issued, no paid work performed.**
- **All bindings re-verified on the final clean commit and none moved:** executable source `cad571c8…` (56 files, `PHASE_B_UNCOVERED` empty), preregistration `ba572bb7…` (self-hash and stated value agree, and it binds that digest), session plan `23f23649…`, science plan `02be33b9…`, both calibration profiles by spec *and* content, and all 11 historical probe reconstructions still verify. The state-only commit moved nothing it should not have.
- **The blocker: `ckpt_store_capacity_gate` fails by 1.03 GiB.** `/home/ecs-user/aad-artifacts` has **10.52 GiB** free; landing the five Phase-B Top-5 leaves needs **11.55 GiB** — 5.55 GiB of leaves plus a **measured** 6 GiB working margin, which exists because the dev box ran out of disk mid-suite on 2026-08-20 and produced fourteen errors that were not code failures. The gate is asked *before a pod exists* precisely so a search that succeeds and cannot come home is impossible; that is the attempt-11 loss.
- **I did not shrink the margin to pass the gate.** It is measured, not chosen, and compensating around a fail-closed gate one step before spending $26 is the failure mode every one of these gates exists to prevent.
- **No safe unilateral fix exists, and the most obvious candidate is the most dangerous.** `relay_retired/` (3.0 GiB) looks like retired duplicates and is the opposite: the 2026-08-22 retention change deleted those objects' **remote** mirrors on the explicit basis that "the checkpoint itself continues to exist canonically on local storage", so the local copy is now the **sole surviving copy**. Deleting it would destroy artifacts, not reclaim redundancy.
- **The other candidates are equally not mine to take.** The ~3.4 GiB of `MinerU2.5`, `livekit turn-detector` and `plugins-persona` in the HF cache belong to the **sibling AlphaAvatar project**, and the 2026-08-21 derived-cache cleanup recorded that it deliberately did not touch them. The review-tier checkpoints (23.09 GiB) were explicitly **retained** by that same maintainer decision, which chose derived-cache cleanup *instead of* checkpoint deletion. Both provably-redundant trees — `wheelhouse_vllm_cp312` and `ladder_uniform`, the only two the mirror-verification record covers — were already reclaimed. The box is a single 197 GB filesystem at 95%; there is nowhere else to put the store.
- **The authorization was deliberately NOT issued.** Step 4 precedes the gates in the instruction order, but a `PhaseBAuthorization` is one-use and binds `authorized_session_commit`; any fix for the disk will add a commit and invalidate it. Issuing now would burn a grant and leave an artifact asserting permission for a launch that cannot happen.
- **Also recorded:** a comment in `autoinit_phase_b_launch.py` claims the beam-6 correction reserve "doubles with" the base search allowance. It does not — `phase_b_budget` widens only the search phase, so the reserve stays at Phase A's 36.2158. The **behaviour is sound and conservative** (deadline 543.9841 min = 9.07 h against a priced P=2 maximum of 7.51 h), but the comment overstates it. Not corrected here: that file is inside the executable-source digest, and editing it would move `cad571c8…` after the bindings were verified. To be fixed in the same pass as any other post-Phase-B change.
- **Revisit when:** the maintainer decides how to free ~1.03 GiB. Then re-verify the digest, issue the one-use grant against the new commit, re-run the gates, and launch.

## 2026-08-26 — Three paid-path defects the reviewer found, repaired

- **Context:** a narrow repair pass on the Phase-B executables. No pod, grant, authorization, paid session or budget increase. Phase B is **not** launched. Frozen Phase-B science, the P=2 search space, the candidate set and the Stage-0 comparability contract are unchanged.
- **Defect 1 — the cross-phase candidate universe was SIX, not eight.** The inherited `stage2` built `self.leaves + control`, and journal seeding does not fix that: a citation is only consulted if a **descriptor** exists for its candidate, and descriptors come from that list. The run would have seeded eight citations, used three, and compared a different set from the one the preregistration froze. **Fixed** with a `candidate_universe()` seam in the Phase-A driver — used by stages 2, 3, 4 *and* the leaf-retention record, so a cross-phase candidate cannot appear in one selection and vanish from another — plus a Phase-B override returning `leaves + imported finalists + control`. The two finalists are injected by digest and **measured inside the search**, where the teacher and primed evaluator already exist; measuring them afterwards would mean reloading a 4B teacher the device-handoff contract has just released. `self.leaves` still means exactly the Phase-B Top-5.
- **Their ids are the original canonical ones**, because `probe_configs` derives probe ids from `state_id[:12]`; a renamed candidate would stop matching the historical records that are its entire evidence. `make_retained_state` is a new constructor for this — `make_control_state` prefixes `control-` and stamps `retained_canonical`, which is what makes something advance unconditionally and count as the baseline, and it now **refuses** that provenance so an imported candidate cannot masquerade as the control.
- **Fail-closed, twice.** `require_citable` refuses **before the search** if any imported finalist lacks verified evidence for a rung it will be asked for, and `make_retained_state` refuses bytes whose digest contradicts the record. Neither falls through into training a checkpoint staged read-only for measurement.
- **Defect 2 — disk.** The launcher inherited Phase A's `--disk-gb=200` against a frozen 244.87 GiB peak working set. Default is now **300** with a `disk_provision_gate` that refuses anything below it before a pod exists. No canary: the requirement is mechanically known.
- **Defect 3 — the successful-run transfer contract.** `fetch_products` was `finalists_to_fetch` (canonical-id **strings**) while `products_secured` was `selected_leaves_secured` (transfer **records**). It could fail only after a scientifically successful run, or leave the P=2 Top-5 bytes unsecured. **Fixed** by wiring the proven `fetch_selected_leaves`, and `fetch_finalists` is deliberately *not* chained: for Phase B its retention record also names the two imported finalists, already retained off-pod. The launch-time `ckpt_store_capacity_gate` is restored to the prechecks.
- **And the same defect one layer down:** `selected_leaves_secured` called `.get` on whatever arrived, so the mismatched wiring would have raised `AttributeError` rather than refusing — at teardown, after success. It is now defensive on **shape** (not on outcome): a non-mapping entry is not evidence that a leaf is secured, and the missing-leaf branch reports it.
- **Only the two admitted finalists are staged**, filtered from the transport manifest. Staging all five would put 3.3 GiB of checkpoints the session must not compare onto the pod.
- **Cost is unchanged: ceiling `$26.8049`, floor `$13.0800`, implied cap `$256.8399`.** Eight candidates at `sa`, five newly probed. The eight-candidate universe is not an eight-probe bill.
- **Two of my own tests were identity-fragile and are recorded rather than quietly fixed.** They compared functions with `is`, and `test_phase_a_rehearsal` loads the launcher a second time through `spec_from_file_location` — so two module objects hold two function objects for the same source. They passed alone and failed in the full suite for a reason unrelated to the wiring under test; they now compare by name, and both still kill their mutations.
- **Verification:** full suite and frozen verifier; **12 mutations** on the three repaired paths, each turning a passing state into a failing one and each checked against its *targeted* test. Two initially survived — stage 2 bypassing the seam, and the secured gate ignoring `rc`/`matched` — and both tests were strengthened rather than the mutations dismissed.
- **The preregistration id was also churning and is now stable.** `generated_utc` was inside its own hash, so every regeneration produced a new id even when not one byte of the commitment had moved — an identity that cannot be cited as "the frozen preregistration". The timestamp stays in the document as provenance and is excluded from the hash, by the writer and by the launcher's gate alike. Regenerating twice now yields the same id.
- **Identities re-frozen:** executable source `57ca2b96…` (56 files, nothing uncovered), preregistration `ba572bb7…`, session plan `23f23649…` unchanged, science plan `02be33b9…` unchanged. Earlier digests from this session's iterations are **superseded, not current**. Both historical harness digests moved again — phase_a `2f8082fe`, continuation `3eaefd15` — because both sets contain the two files this pass edited; both sessions are complete with consumed authorizations.
- **Remaining blocker:** none in the code. A maintainer cumulative-budget decision and a reviewer GO.
- **Revisit when:** that decision is taken.

## 2026-08-26 — The Phase-B executables exist; the last code blocker is closed

- **Context:** the remaining `$0` Phase-B implementation — the pod driver and launcher — then re-freezing the identities that bind them. No pod, grant, authorization, paid session or budget increase. Phase B is **not** launched.
- **The driver subclasses Phase A's and overrides four things.** The governing artifacts (a `PhaseBAuthorization` and the Phase-B plan, both class attributes); stage 0's *additional* bindings; the joint two-profile search; and `restore_probe`. Everything else — the affordability arithmetic, the CPU budget, the suite binding, the leaf-shortfall rule, the durability boundary, the probe runner, the battery, the selection rows, the journal and teardown — is inherited, because a Phase-B copy of the parts that spend money would be a second implementation of them. A test asserts the constructor leaves **no attribute the inherited methods read** unset, since it deliberately does not call `super().__init__`.
- **Reuse is implemented as journal seeding, not as a special case.** An imported probe becomes a file in the probe journal, after which the inherited machinery treats it exactly like one this session ran: `restore_probe` finds it, `run_rung` skips the budget check, `pooled_over_rungs` pools it. The ids match by construction — `probe_configs` derives them from the **science** plan id, which Phase B reuses unchanged.
- **A seam was extracted from the Phase-A driver rather than duplicating 160 lines.** `stage1` now calls `self.run_search(...)`; Phase B overrides that method and nothing else in stage 1. Behaviour-identical for Phase A, and its own end-to-end stage test still passes. **This moves BOTH historical harness digests** — phase_a `a182efb4 -> 92fc1bd3` and continuation `a27bb922 -> 2b182037` — because `RECOVERY_CONTINUATION_HARNESS_FILES_V1` also contains `autoinit_phase_a_driver.py`, the class its own driver subclasses. I initially recorded only the Phase-A movement; both are reported. Both sessions are complete with consumed authorizations, so a moved digest means either would now refuse to relaunch, which is the desired direction. No frozen SCIENCE identity moved.
- **Two real defects were found while building, both by tests rather than by reading.** (1) The v2 mixture was declared as a `RelayInput`; it was built on the dev box and never uploaded, so the pod could not have fetched it — **exactly what killed Phase-A attempt 5 at $0.6426.** It now travels as a `LocalAsset`, 780 KB over a measured 0.72 MB/s uplink. (2) The inherited `mark()` appends to a module-global `STATUS`, so a Phase-B run would have written its markers to `autoinit_phase_a.status` while the launcher polled `autoinit_phase_b.status` — **an entire session looking hung and being killed.** The driver now redirects that global, and a test asserts driver and launcher agree on the path.
- **The executable-source identity is re-derived, not extended by hand:** the transitive import closure of the driver and launcher, plus what they shell out to and what the package registers, minus what another bound identity already covers. **56 files, digest `39ba45d8…`, set version 2, and `PHASE_B_UNCOVERED` is now empty** — the launcher's precheck refuses to create a pod while anything is listed there. `recovery.py` stays delegated to `recovery_scoring_contract`, which the driver binds at stage 0. `autoinit/generation.py` and `generation_compat.py` were **added**: they are not in `GENERATION_SOURCE_FILES_V1` (that set is the evaluator) and they decide the protocol hash and the comparability verdict the whole Stage-0 gate turns on.
- **`expected_usd` is renamed `planning_floor_usd`,** as instructed and before any grant exists. No expected-value assumption over survivor identity or tie-break probability is defined anywhere, so calling the low figure "expected" would invite budgeting against an outcome whose probability nobody estimated.
- **Unchanged, deliberately:** the frozen Phase-B science and preregistration semantics; the Stage-0 fail-closed comparability rule; the cross-phase candidate set. The three excluded Phase-A leaves are still excluded, and a test now proves the **driver's own** guard holds even when handed a reuse record that admits one — previously the exclusion was defended twice and a mutation removing the driver's filter changed nothing observable.
- **Pricing, re-run mechanically after the freeze:** low **$13.0800** (a floor), **authorization ceiling $26.8049**, implied cumulative cap **$256.8399**. The launcher's ten priced probes match the pricing artifact's ten. The 14-probe no-reuse figure remains a rejected counterfactual.
- **Verification:** 8 mutations across the driver and launcher gates, each turning a passing state into a failing one, and each checked to fail its *targeted* test — one initially did not, and the test was strengthened rather than the mutation dismissed.
- **Roadmap recorded, not started:** Phase B is the terminal evaluation of **AutoInitializer v1**, not the end of the research. After it closes: the generalization pass first, then a DEPTH **incumbent** (a default for the next preset — historical implementations stay registered and reproducible, and universal superiority may not be inferred from one winning composed leaf), then ATTENTION → FFN → RESIDUAL_WIDTH. Every new method gets a new immutable id, and bake-offs are followed by a joint interaction confirmation search. `docs/POST_PHASE_B_GENERALIZATION.md`.
- **Remaining blocker:** none in the code. A maintainer cumulative-budget decision and a reviewer GO.
- **Revisit when:** the budget decision is taken.

## 2026-08-25 — Phase B is frozen: plan, grant type, source identity, preregistration, price

- **Context:** the remaining `$0` Phase-B preparation, against the reviewed implementation. No pod, grant, authorization, paid session or budget increase. Phase B is **not** launched.
- **Decision (1) — comparability is a TERMINATE condition, not a larger run.** `generation_runtime_comparability@v2` is not merely a historical-probe reuse check: it is what permits behavioural results to be judged against the **frozen** Stage-3 feasibility floor and equivalence interval, which were materialized under Phase A's runtime. If Stage 0 finds the runtime not comparable, those thresholds describe nothing this session could produce, so it **fails closed and terminates before any search or probe**. It does **not** answer failure by re-running all eight candidates — that is a differently-thresholded experiment wearing Phase B's name, and it would not restore the interval either. Stage-3 controls are not rematerialized and thresholds are not redefined under any outcome.
- **Consequence for the budget:** the 14-probe no-reuse figure (`$33.3529`) is demoted to a **rejected counterfactual**, retained in the pricing artifact only so the rejection is on the record. The **authorization ceiling is `$26.8049`** — the verified-reuse hard case — implying a cumulative cap of **`$256.8399`**. `$13.0800` remains a **floor**, not an expectation. No cap increase is requested here.
- **Decision (2) — Phase B gets its own executable-source identity; Phase A's is untouched.** `PHASE_A_HARNESS_SOURCE_FILES_V1` is historical fact about a completed experiment and is not widened. `PHASE_B_EXECUTABLE_SOURCE_FILES_V1` is **28 files**, digest `92e2170e…`, derived from the import closure of the search entry point and then curated. It includes the adapter package — which the AST closure of `search.py` alone never reaches, because the package `__init__` registers it.
- **Provenance closure, not a maximal list.** Deliberately absent, each for a stated reason: `reweight.py` and the mixture builder, because **the pod never runs them** — it consumes a materialized artifact bound by `profile_hash` + `content_sha256` + `items_file_sha256`, so they are artifact provenance; and the probe path, which is already bound by `trainer_source_digest`, `generation_source_digest` and `recovery_scoring_contract`. `PHASE_B_DELEGATED_IDENTITIES` names which identity covers what, so "not in the digest" never means "unaccounted for".
- **Named gap rather than silent omission:** there is **no Phase-B pod driver or launcher**. `PHASE_B_UNCOVERED` says so, and they enter the digest when written. This is the one material blocker between here and a fundable GO.
- **Decision (3) — the candidate set is closed before any Phase-B result exists.** Phase-B Top-5 + the two retained Phase-A finalists + the canonical control = 8 at `sa`. The three Phase-A non-finalists stay **excluded** although their `sa` probes are already paid for and verified: zero marginal cost is not an admission criterion, they lost the Phase-A rung-1 behavioural admission step under the same primary metric and gates, and a set that can grow once results are visible is not a preregistered set. Recorded in the plan so it cannot be reopened.
- **Decision (4) — the preregistration binds BOTH calibration identities.** `profile_hash` fixes the specification; `content_sha256` (with `items_file_sha256` retained) fixes the sampled bytes. Neither implies the other, and for v2 the seed does not even reach the bytes. `PhaseBAuthorization.require_calibration` refuses a mixture whose spec matches and whose content does not.
- **Decision (5) — `PhaseBAuthorization` is a distinct type whose `allows_phase_a` is False BY TYPE.** Not a subclass of the Phase-A grant or of `SpendAuthorization`, and `load()` refuses a Phase-A artifact by schema. Phase A is complete; nothing issued for Phase B can reopen it.
- **The preregistration** (`8f211ce1…`, self-verifying) freezes: both profile identities, the joint P=2 search, beam 6, `SCHEDULE_V1`, `PARETO_V1`, unchanged `state_eval@v1`, the operator branching rule with its reasons, the executable-source digest, the Stage-0 fail-closed condition, the Top-5 admission rule, the imported finalists and control, the strict-reconstruction requirement, `sa → global searched Top-2 + control`, pooled `sa+sb` equivalence, conditional `sc`, no fourth seed, `unresolved_equivalence` as a valid terminal result, and — explicitly — that **search-side KL/NLL and the canonical Stage-1 NLL may not break a tie**.
- **Recorded, not started: the generalization backlog** (`docs/POST_PHASE_B_GENERALIZATION.md`). Core describes mechanisms, adapters describe model families, presets describe data/experiment choices, session plans describe execution. Highest priority is moving the causal-LM round-trip assumption out of `search.py` into the adapter. **None of it may precede Phase B** — mixing an architecture rewrite into the calibration experiment would make its result unattributable. `ArchSpec`, `ArchitectureAdapter`, capability dispatch, the open registry and generic search over `target_spec` are explicitly not to be redesigned.
- **Verification:** **7 mutations** on the new Phase-B gates, each turning a passing state into a failing one — widening the Phase-A harness, dropping `search.py`, bloating the set with the non-executing builder, answering comparability failure by re-running, a grant claiming Phase A, `require_calibration` checking only the spec hash, and admitting an excluded leaf. Each was additionally checked to fail its *targeted* test rather than only the preregistration cross-check, which catches source drift and would otherwise mask a weak assertion.
- **Risks:** the driver does not exist, so the digest that an authorization would bind is not yet the digest of the thing that will run. Issuing a grant before the driver is written would bind the wrong executable.
- **Revisit when:** a Phase-B driver/launcher exists and a maintainer takes the cumulative-budget decision.

## 2026-08-25 — Phase B: v2 materialized, reuse verified, and a rule whose seed does not reach the bytes

- **Context:** the reviewer approved `multihop_qa` = 7,074 and required two corrections before materialization, plus a repair of the pricing contract. No pod, grant, authorization, paid work or budget increase. Nothing launched.
- **The approved rationale, stated narrowly as instructed.** 7,340 does **not** make the entire activation-statistics input rank-deficient. It concentrates the `multihop_qa` capability slice into four copies of one session — an unnecessary **item-level concentration confound**. 7,074 removes it for 452 positions absorbed by `rag_evidence` *inside* `rag_multihop`, so every domain quota is unchanged.
- **Correction 1 — the seed is wired in, and its real reach is recorded.** R5's final tie-break is now the seed-derived order `sha256(f"{seed}:{item_id}")`, not lexicographic, and the complete R1–R5 semantics are stated verbatim in `sample_rule` **before** `profile_hash` is computed. That is the structural fix. The honest result: **on this pool the seed never binds.** Every sub-type optimum is unique — verified by brute-force enumeration, not merely by the dynamic program that produces it — so a different seed yields the same bytes. The seed identifies the *rule instance*, not the sampled mixture. Asserted as a test so it cannot rot into a claim, and it is exactly why the preregistration must bind `content_sha256`.
- **Correction 2 — the two repair levels are deliberately different rules, and a unified one was rejected on measurement.** A domain's deviation distorts the weights the profile declares, so R2 minimizes it (nearest reachable, ties lower). A sub-type's deviation is absorbed by a sibling and moves no domain weight, so R4 spends it on support. A single "max support" rule at both levels moves quotas that are currently **exact** — `gsm8k` 10,560→10,556, `openmath` 10,357→10,347, `code` 11,953→11,939 — trading real deviation for support where the arithmetic never forced a choice.
- **v2 is materialized:** `profile_hash 6c67b8df…`, `content_sha256 cdb28389…`, `items_file_sha256 a2ff8c92…`; 62 draws over **51 of 67** sessions; **59,763 positions exactly**; maximum domain deviation **0.70 positions**. Leakage is **inherited, not re-proved** — every token is a token of the source mixture drawn with replacement. `manifest_sha256` is deliberately not pinned: it covers `generated_utc` and moves on every rebuild.
- **The pricing contract is repaired.** Reuse is no longer inferred from a file existing. `verify_historical_probe_reuse.py` runs the strict check — completeness, the frozen seed, the **artifact digest re-derived from the retained checkpoint bytes**, the battery and scoring-contract identities, the attested protocol hash — and **all 11 probes verify**. `price_phase_b.py` consumes that verdict and **fails closed** if the record is missing, unverified, or describes different probe bytes.
- **One leg cannot be closed at `$0`, and is a scenario rather than a hedge.** Phase B's own runtime must be comparable to Phase A's under `generation_runtime_comparability@v2`, and Phase B's runtime does not exist yet. If that Stage-0 precondition fails, **all** reuse is lost at once. So: **low 5 probes $13.0800** (a FLOOR — no expected-value assumption is defined anywhere, so nothing is called "expected"), **hard-with-reuse 10 probes $26.8049**, **hard-no-reuse 14 probes $33.3529**. The last is what an authorization must cover. Implied cumulative cap **~$263.39** against $230.0350 spent.
- **Also surfaced, not acted on:** the three unadmitted Phase-A leaves hold **verified** `sa` probes and are retained off-pod, so admitting them would cost nothing at `sa`. The procedure excludes them; that remains the reviewer's call.
- **A test hole found by mutation and closed.** Every v2 test read the *built artifact*, so forcing the sub-type repair back to NEAREST — which would move `multihop_qa` to 7,340 — left them all green. The artifact was right; the code no longer produced it. A rebuild-from-pool test now re-derives `content_sha256` and kills that mutation, which is what P4 means by reproducible.
- **Not done, deliberately:** no separate paid session to measure the activation-statistics GPU/CPU split. The conservative bound stands unless the measurement falls out of the Phase-B run itself.
- **Surfaced this pass, not taken: the Phase-A harness digest does not cover what a paid search computes.** This pass changed `search.py` and `calibration.py` — the sentinel fix and the v2 profile, both of which change a paid run's output — and **both harness digests were unchanged**. `PHASE_A_HARNESS_SOURCE_FILES_V1` names `phase_a_search.py` but not `calibration.py`, `search.py`, `ranking.py`, `reweight.py` or `operators/base.py`. Extending the set moves the digest and is properly part of the Phase-B plan (item 9), so it is recorded here rather than done.
- **Revisit when:** the Phase-B plan, authorization type and preregistration are written — in that order, because the preregistration binds both.

## 2026-08-25 — Phase B: the reviewer's decisions, and what executing the P=2 path immediately found

- **Context:** the reviewer resolved all three open choices from the 2026-08-24 reconstruction and gave a narrow GO for checklist item 7 only (a `$0` toy P=2 search). No GPU, pod, grant, authorization or budget increase. Nothing was launched.
- **Decision (1) — the mixture is option (b):** a versioned `calib.reasoning_heavy@v2` doing deterministic **with-replacement** reweighting of the same frozen 67-item pool, preserving the 59,763-position budget, the declared domain weights and the existing leakage-safe support. No silent truncation, no silent approximation.
- **Decision (2) — §6.3 is the full fresh joint P=2 beam**, exactly as priced. Phase-A leaves are **not** reused to restrict the search space, because P=2 changes beam pruning and the domain-balanced arm must compete jointly.
- **Decision (3) — Phase B is not search-only.** Its terminal procedure is a cross-phase behavioural selection: Top-5 P=2 leaves + the two retained Phase-A finalists + the canonical initialization control; historical sa/sb/sc reused only after strict reconstruction proves the same materialized recovery protocol and seed; no fourth seed; `unresolved_equivalence` remains a permissible result. Search-side KL/NLL and the later canonical Stage-1 NLL may **not** break a tie.
- **Finding (1) — exact realization of the reweighted mixture is impossible, for a reason prior to any algorithm:** four of the five domain targets are non-integers (`0.35 × 59,763 = 20,917.05`). Largest-remainder apportionment resolves that deterministically and sums to 59,763 exactly; 4 of 5 domain quotas are then realizable by whole items, `code` = 11,953 is not, and the repair moves it to 11,952 with the position transferred to `general`. **Minimum achievable deviation is 0.70 positions — 1.17 × 10⁻⁵ of the budget.** It cannot be zero.
- **Open, and deliberately not taken:** `multihop_qa` (5 items, all ~1,675–1,835) couples support to deviation. The nearest-reachable rule picks 7,340, which realizes the sub-type as **four copies of one session** — rank-deficient input to an activation-statistics operator. 7,074 buys 4 of 5 distinct sessions for 452 positions confined inside `rag_multihop` (0.76% of the budget), with every domain weight still exact to 0.70 positions. **Recommended, and held for confirmation** under the reviewer's own "propose the rounding rule before implementing the profile" instruction. Full frontier: `autoinit_phase_b_reconstruction.md` §2A.
- **Also rejected on measurement:** apportioning at sub-type level to preserve within-domain structure. It makes `multihop_qa` = 7,526 unreachable by 186 positions, and repairing that drifts the **domain** weights by ±186 — defeating the profile's entire purpose. Confining every sub-type repair inside its own domain is what preserves them.
- **Finding (2) — executing the P=2 path for the first time found a pod-fatal defect.** `BeamSearch.calibration_for` calls the loader with the `calib.none@v1` sentinel, whose `resolve()` raises by design. Every loader ever written here — every test's, and `phase_a_search.py`'s — was `lambda profile: items`, which ignores its argument and answered anyway. The first loader that actually dispatched on profile raised inside the beam; a correct Phase-B loader delegating to `profile.resolve()` would have raised `CalibrationError` there instead, **on a paid pod, mid-search**. Fixed by short-circuiting the sentinel. **No identity moves:** `n_calibration_items` is the only thing derived from that list, `_expand_one` explicitly excludes it from `config_hash`, and both `CalibrationNeed.NONE` implementations ignore `config`; 1192 autoinit/pod tests pass unchanged.
- **Decision (4) — the entry-point seam is repaired, and the harness digest moves.** `run_phase_a_search` takes `profiles=` alongside `profile=`; `resolve_profiles` and `build_calibration_loader` resolve the searched profiles and their items **together**, so the label and the mixture can no longer disagree. `scripts/autoinit/phase_a_search.py` is in `PHASE_A_HARNESS_SOURCE_FILES_V1`, so the **Phase-A harness digest moves** — expected, authorized, and reported. The continuation digest `a27bb922` is unchanged.
- **Repricing, mechanical and not an authorization:** `scripts/autoinit/price_phase_b.py` reads the historical probe inventory off disk (11 records: 6 sa, 3 sb, 2 sc) rather than assuming it. Of 8 sa candidates, 3 are already observed, so **5 sa probes are owed**; sb costs 0–2 and sc 0–3, priced as two *internally coherent* worlds rather than by mixing an optimistic sb with a pessimistic sc. **5–10 probes, expected $13.0800, hard $26.8049**, implying a cumulative cap near **$256.84** against $230.0350 spent. Note the control has **no sc on record**, which is what makes the worst-case sc cost one more than sb.
- **Surfaced, not acted on:** the other three Phase-A leaves (`158b96cf`, `281a02c3`, `4e429f7e`) also hold sa probes and are retained off-pod at 1.2 GiB each. The procedure admits only the two finalists, so that evidence goes unused — cheap for a reviewer to revisit, invisible if unstated.
- **Verification:** 18 mutations across five new gates, each turning a passing state into a failing one. One of my own assertions was found **vacuous** — `profile_id` is inside `profile_hash`, so "distinct profiles have distinct hashes" could never fail — and was replaced with one asserting `_stats_key` directly, which dies when the profile is dropped from the key.
- **The seam-testing lesson, paid for again.** The `phase_a_search.py` repair was unit-tested at the extracted seam (`resolve_profiles`, `build_calibration_loader`): 8 tests, 4 mutations, all green — and the rename those helpers were extracted from left a dangling `active_profile` in the **summary block**, a line only a search that runs to completion reaches. The seam tests could not see it and the whole-suite run caught it. Testing the injection points is not testing the function; `test_the_whole_function_runs_a_two_profile_search` now drives P=2 through `run_phase_a_search` itself, and restoring the dangling reference kills it.
- **A consumer contract preserved while extending it:** `summary["calibration_profile"]` is read by `test_phase_a_stages1_5_execute`. It stays singular and truthful — the one profile on a P=1 run, and **`None`** on a P>1 run rather than the first of several, because naming one of two mixtures is the same class of lie the loader seam was fixed to prevent. `summary["calibration_profiles"]` is the always-correct list.
- **Risks:** the `multihop_qa` choice is the last thing standing between here and a materialized v2, and everything downstream (plan, authorization type, preregistration) binds v2's profile hash, so it cannot be parallelized away.
- **Revisit when:** the `multihop_qa` choice is confirmed.
...
- **Decision — the `phase_a_search.py` mislabeling defect is reported and deliberately NOT fixed.** `active_profile` and `calibration` are resolved independently, so `profile=X` without `calibration_items=` would label a run X and feed it `domain_balanced` items. Unreachable today — every call site passes both or neither — and reachable the moment a second profile is wired. Its file is inside `PHASE_A_HARNESS_SOURCE_FILES_V1`, so repairing it now would move the Phase-A harness digest for no present benefit while no run is authorized. Whoever wires the second profile must fix it in the same change.
- **What is already sufficient:** `search.py` branches every calibration-consuming implementation over N profiles and keys both the calibration and activation-statistics caches on `profile_hash`; `cost.py` prices P=2; and every source asset is present and hash-verifiable, including the teacher tokenizer at its pinned revision, so a rebuild is CPU-only and free.
- **Finding (3) — "the engine supports P=2" rests on an enumeration, not on an executed search.** The P=2 test asserts `_candidate_expansions` returns 2 sentinel + 4×2 calibrated = 10 and then stops; it never calls `run()`. Both dry-run journals are P=1, every other `make_search` caller is P=1, and every test passes `calibration_loader=lambda profile: items` — a closure that ignores its argument, so **the per-profile loading path has never run with two distinct item sets.** Given that four paid pods in this project have died in never-executed lines, a toy P=2 run through the full materialize → reload → validate → measure cycle is a `$0` prerequisite before any Phase-B pod. It is the one checklist item that is **not** blocked on the reviewer.
- **Cost, mechanically derived, and NOT an authorization:** search P=2 beam 6 warmup 1 on an L40S is **$1.8950–$7.4349**, 1.91–7.51 h, 74–114 states, 8–20 leaves, **244.9 GiB** working storage (provision ≥300 GiB), 14.3 GiB resident. The range is still the unmeasured activation-statistics GPU/CPU split. With a $3.00 reserve the indicative hard totals are ~$10.44 search-only, ~$25.17 with Phase-A-shaped recovery, ~$30.08 with the conditional third seed — implying a new cumulative cap of roughly $240.47 / $255.20 / $260.11 against **$230.0350** actual spend. **$3.9650 remains and funds nothing.** The binding per-launch ceiling must come from the pricing and authorization code once a plan exists.
- **Risks:** the three open choices are not independent — the mixture-draw decision changes what the search measures, which changes whether a search-only result is worth having. Deciding them piecemeal is how a Phase B gets funded and then produces a number nobody can act on.
- **Revisit when:** the reviewer settles the three choices in §6 of the reconstruction. Items 4–8 of its checklist are then straightforward `$0` work.

## 2026-08-24 — Phase-A closeout: what it selected, what it did not decide, and what comes next

- **Context:** a `$0` closeout on the completed Phase A, before Phase B is opened in a fresh session. No GPU work, no grant, no science redesign.
- **The launcher defect is fixed at its narrowest point.** `session_runner` now computes `all(fetch_result_ok(f) for f in fetched)`. `fetch_result_ok` judges a **transfer result** by `rc == 0` and treats an **identifier** (`finalists_to_fetch`'s `canonical_id` strings, which transfer nothing because those bytes are already off-pod) as nothing to fail. A dict without `rc` is *not* a success. Whether a session secured what it owes remains a separate question answered by `products_secured`, precisely because `all([])` is vacuously true. Artifact-selection policy and Phase-A science are untouched. **3 mutations**, each turning a passing state into a failing one: the crash restored at the call site, failed transfers waved through, and a missing `rc` defaulted to 0.
- **Phase A is recorded as a SELECTION experiment, consistently.** It searches initialization and operator order under the fixed `calib.domain_balanced@v1`, and its 0.86M matched probes are **selection probes to distinguish initialization quality** — not the project's recovery training, which remains later Stage 2/3 work. The small absolute correctness rates are **not** recovered-model capability; they sit near the floor in every arm including the control, exactly as the Stage-3 controls do at 0.0118.
- **The result stays as the plan returned it.** `unresolved_equivalence`, `winner: None`. `cca699c93f34…` is the **numerical/provisional leader** and **is** separated from the canonical initialization (0.020588 > 0.011695); it and `85bde4ded2c3…` were **not separated from each other** after the full frozen sa/sb/sc procedure. **Neither searched leaf is described anywhere as the resolved Phase-A winner.**
- **The two "controls" are kept distinct in every document.** `preflight_ctl_r0860k_{sa,sb}` are the historical permanent Stage-3 controls that materialized the thresholds and were **imported, never retrained**. `qwen3_0p6b_init_v0` is the Phase-A canonical **initialization** control, hash-injected, which correctly received matched fresh selection probes. Conflating them was raised as a possible protocol blocker during attempt 7 and resolved against the live execution path.
- **The next sequence is recorded and NOT started:** (1) **Phase B** — calibration/data-distribution sensitivity, planned `calib.reasoning_heavy@v1` against `calib.domain_balanced@v1`, because Phase A searched one distribution only and its ranking is conditional on that; (2) complete **final initialization selection** after Phase B; (3) the **canonical Stage-1 NLL diagnostic** on the finally selected initialization under the established Stage-1 contract — **diagnostic only, never a promotion criterion**, and **not** the `state_eval@v1` search-side NLL fields, which are a different measurement; it has never been run; (4) only then the formal **Stage 2/3 recovery training**.
- **Budget, re-derived and normalized:** `$230.0350` spent of `$234.00`, **`$3.9650` remaining, which authorizes no new paid session.** Phase B or any subsequent GPU work needs a new explicit maintainer budget decision.
- **Revisit when:** Phase B is reviewed and authorized from a fresh session.

## 2026-08-24 — PHASE A IS COMPLETE: unresolved_equivalence, and no fourth seed

- **Context:** recovery continuation attempt 7, the seventh authorized continuation and the last the budget could fund, ran 779.3 min for **$12.8587** — under the $14.9233 expected — and reached `ALL_DONE`. Six stages, **eleven probes trained and scored**: 6 in rung 1 on seed sa, 3 in rung 2 on sb, 2 in the conditional rung 3 on sc.
- **The result, and what it does not say.** `decision_status: unresolved_equivalence`, `winner: None`, `tie_break_ran: true`. Pooled `correct_overall`: `cca699c93f34` **0.029412**, `85bde4ded2c3` 0.019608, `control-qwen` 0.008824, against the frozen interval **0.011695**. The two searched leaves differ by 0.009804 — inside the interval, so they remain unresolved even after the third seed, and **no fourth seed follows**: that is the frozen rule and `unresolved_equivalence` is a result rather than a condition to resolve. `cca699c9` **is** separated from the canonical control (0.020588 > interval) and is also ahead on the behaviour axis (0.6561 vs 0.4947 usable rollout); `85bde4de` is not (0.010784). **This record claims no win.** The plan's own winner rule returned `None`, and absolute correctness is small in every arm including the control — a property of the 0.86M recovery rung, not of the comparison. What follows scientifically is the maintainer's call.
- **Comparability held.** `comparable_identity 70a26e0b…` live equals historical, `bound_to_stage3_thresholds: true`. The run's raw `evaluation_protocol_hash 7327e880…` differs from the frozen `250f72ef…` only in the driver patch (580.178.04 vs 580.159.03), which `generation_runtime_comparability@v2` declares non-material — the rule that exists so comparability is not a host lottery, doing exactly its job.
- **Every carried-in repair held on hardware.** The device handoff freed to `0.01 GiB` with `live_retention: false`; the checkpoint resolver found every probe; and the evaluation tokenizer materialized (`copied, copied, copied`) before all eleven batteries, with `tokenizer_source` remaining *"the evaluated checkpoint"*. Probe training ran **61.0–61.1 min** against the **61.55** priced, so attempt 6's 71.9 min was host variance rather than a systematic underestimate — the pricing basis is confirmed.
- **The launcher then crashed after collection, and it is a real defect.** `session_runner.py` computes `all(f.get("rc") == 0 for f in fetched)`, but Phase A's `fetch_products` is `finalists_to_fetch`, which returns **`canonical_id` strings**, not transfer-result dicts. `AttributeError` — on a line **only a successful Phase A reaches**, which is precisely why six prior attempts never hit it. Consequence: the session records `passed: false` / `INCOMPLETE` despite `DRIVER_EXITED:0`, the teardown gate was skipped and teardown ran as an emergency delete. **A successful Phase A cannot currently be recorded as successful.** Not fixed here — nothing further is authorized.
...
- **Revisit when:** the maintainer decides what `unresolved_equivalence` means for the recipe, and whether to fix the launcher defect. Full record: [`autoinit_recovery_continuation_attempt7/`](../runs/unscoped/recovery_continuation/attempt7/).

## 2026-08-23 — The evaluation tokenizer is materialized, not redirected

- **Context:** the maintainer accepted the comparability diagnosis and directed a narrow producer/consumer repair in the driver — **not** `--tokenizer`, which would change the material `tokenizer_source` rule and make the recovery probes incomparable with the Stage-3 controls.
- **Decision:** after `trained_model_dir()` resolves a trained probe and before `battery()` is called, `materialize_eval_tokenizer()` copies exactly `tokenizer.json`, `tokenizer_config.json` and `chat_template.jinja` from the already-attested `CANONICAL_INIT` into the checkpoint's `model/` directory. `battery()` continues to invoke `uncapped_eval.py` with `--model <trained dir>` and **no `--tokenizer`**. The frozen rule *"the evaluated checkpoint"* is therefore left alone and made **true**, and the bytes are the ones Stage 0 attested.
- **The helper is fail-closed and idempotent, in that order:** a missing source sidecar raises, because a probe scored against a half-present tokenizer is worse than one that did not run; an absent destination is copied byte-for-byte and re-hashed; a destination already identical is accepted, so a re-entered probe does not fail on its own previous work; a destination that **differs** is refused and never overwritten, because otherwise which tokenizer the probe was scored against becomes unrecoverable.
- **Deliberately untouched:** `uncapped_eval.py` — it is part of the generation source digest, so editing it would convert an infrastructure repair into a real protocol change — plus the trainer, `Trainer.save_checkpoint`, the frozen recovery recipe, `generation_runtime_comparability@v2`, the Stage-3 attestation and every frozen threshold. No trained-but-unscored retention machinery was added.
- **Verified:** full suite **2183 passed / 12 skipped** (+7; skips unchanged), frozen verifier clean, and **5 mutations** each turning a passing state into a failing one — the materialization removed (caught by both the wiring gate and, in a 5-minute end-to-end run, the stage-1-5 harness), a conflicting sidecar overwritten, a missing source skipped, and a `--tokenizer` flag added to `battery()`.
- **The end-to-end harness now asserts the substance**, not just the call: it records what `run_probe` actually hands `battery()` and requires all three sidecars to be present and byte-identical to `CANONICAL_INIT`, alongside an argv check that no external tokenizer is passed.
- **Risk that remains:** this reproduces the tokenizer *bytes*, so `tokenizer_sha256` and `chat_template_sha256` should match Stage 0's attestation on the pod. That equality is asserted at `$0` against the dev-box canonical files; the pod's own copies arrive by relay with declared digests, so the two should agree, but the first paid run is what confirms it.
- **Revisit when:** attempt 7 is authorized. Harness digest `8b56fc7b` → `b824441c`. Budget unchanged at `$217.1763` of `$234.00`, `$16.8237` against a `$16.7456` ceiling.

## 2026-08-23 — Attempt 6: the checkpoint read is closed, and the tokenizer fix is a protocol decision

- **Context:** one authorized launch on the repaired tree. Grant commit `948b1e8`, authorization `autoinit.recovery_continuation.2026-08-23T0944Z` bound to it and to harness `0dbf1272`, session commit `08670e5`, bundle round-tripped, all `$0` gates green in the repository's own `.venv`.
- **The attempt-5 repair is confirmed on hardware.** `MARKER:PROBE_TRAINED` at 71.9 min, then `trained_model_dir()` resolved the checkpoint with **no `FileNotFoundError`** and execution entered `battery()` — past the exact line that ended attempt 5. Setup cost `$0.13` and TCP 22 came up in 0.2 min, both the best recorded.
- **Decision: fail closed on the new failure.** Pod deleted at 90.5 min / **$1.4926** with provider confirmation; watchdog `pod_gone` after 91 ticks; evidence collected first. No repair on the live pod, no relaunch.
- **Cause:** `uncapped_eval.py` raised `ValueError: tokenizer.chat_template is not set`, 50 s after training. `Trainer.save_checkpoint` writes `save_pretrained(ckpt_dir/"model")` — weights and config, **no tokenizer** — while `battery()` passes `--model <that dir>` and no `--tokenizer`, a flag whose default is *"the checkpoint's own tokenizer"*. Every previously proven caller (preflight driver, continuation driver, both generation smokes) passed `--model CANONICAL_INIT`, which **is** a full checkpoint carrying `tokenizer*.json` and `chat_template.jinja`. The Phase-A battery is the first caller ever to point `--model` at a trainer-written checkpoint, so the default rule had never met one. Third appearance of the checkpoint-without-a-tokenizer class, after the control-sb packaging repair and attempt 11's one-token tokenizer.
- **The one-line fix is NOT protocol-neutral, and this is the decision the maintainer must take.** Passing `--tokenizer <canonical init>` would clear the crash and silently destroy comparability. Stage 0 of this run attested `tokenizer_source: "the evaluated checkpoint"` and `tokenizer_sha256: c1db93c8…` under the frozen protocol `250f72ef…`; `RecoveryEvaluationProtocol.identity()` returns **every** declared field; and `generation_runtime_comparability@v2` declares material *"every field the protocol already declares except `runtime_digest`, `evaluation_protocol_hash`, `generation_protocol_fingerprint`"*. `tokenizer_source` is therefore material, and changing it to `"external: …"` makes every probe incomparable to the Stage-3 controls that materialized the equivalence interval and feasibility floor.
- **Alternatives, none applied:** (a) `--tokenizer` — rejected above unless accompanied by a new fingerprint version and re-materialized thresholds; (b) the driver places the frozen tokenizer files into the probe's `model` directory after training, so *"the evaluated checkpoint"* becomes true and both `tokenizer_source` and `tokenizer_sha256` are reproduced rather than redefined — driver-local, trainer untouched; (c) the trainer saves the tokenizer at checkpoint time — protocol-neutral too, but changes the trainer and the checkpoint contents, which is outside the standing boundary. (b) appears to be the protocol-neutral driver-local shape, but the choice is the maintainer's.
- **Recorded, not diagnosed:** the probe took **71.9 min** against attempt 5's 61.7 and the **61.55** the budget is priced from, **+16.6%**, on a different host with the same recipe and card model. One observation is not a trend; nine probes at 71.9 would move the expected envelope. Flagged for the next pricing review.
- **The trained probe was again lost with the pod** — 71.9 paid minutes, the second time. Retention remains deliberately unimplemented per the attempt-5 decision.
- **Budget is now the binding constraint.** `$217.1763` of `$234.00` is spent; **`$16.8237` remains against a `$16.7456` ceiling — a margin of `$0.0781`.** One more full-ceiling continuation fits by under eight cents and nothing else fits at all. **Balance is not authorization.**
- **Revisit when:** the tokenizer/protocol decision is taken and a new one-use grant issued.

## 2026-08-23 — The attempt-5 checkpoint-path repair, deliberately small
...
- **Revisit when:** attempt 6 is authorized. Budget unchanged at `$215.6837` of `$234.00`; one `$16.7456` ceiling still fits, leaving `$1.5707`.

## 2026-08-23 — Attempt 5: both memory repairs verified, the first probe trained, and a checkpoint path only 62 paid minutes could reach

- **Context:** one authorized launch on the repaired tree. The chain executed unchanged — grant commit `4794193`, authorization `autoinit.recovery_continuation.2026-08-22T1925Z` bound to it and to the **new** harness `95cf336d`, authorization-only commit `63625e1`, bundle round-tripped with its harness recomputed from the relay checkout, and every `$0` gate green (2172/12, frozen verifier, publisher 11/11 pod-like).
- **Both memory repairs are confirmed on hardware, and the confirmation is quantitative.** Attempt 5's `before` snapshot is byte-identical to attempt 4's — 8,110,229,504 allocated — so the two runs are directly comparable. `freed_allocated_bytes` went from **0 to 8,101,709,824**; allocated after fell from 7.55 GiB to **0.008 GiB**; `live_retention` from **true to false**; driver-visible free from 36.32 GiB to **43.87 GiB**. `require_released` passed because nothing was retained, and `require_headroom` passed on **43.87 against the 43.65 requirement**. That 43.87 is the number the repair predicted *before* the run, to two decimal places — so the 41.65 GiB basis is **very nearly exact**, not merely conservative, and the margin sits where it was intended to sit.
- **The first recovery probe trained.** `MARKER:PROBE_TRAINED` for `rung1.cca699c93f34.sa` at **61.7 minutes**, against the **61.55** the budget is priced from. The pricing basis is now confirmed on hardware rather than inferred, which matters for every future continuation estimate.
- **Decision: fail closed on the new failure.** Pod deleted at 81.9 min / **$1.3511** with provider confirmation; watchdog ended `pod_gone` after 82 ticks; evidence collected first. No repair on the live pod, no relaunch.
- **Cause: a writer/consumer contract gap, writer correct, one of two consumers wrong.** `train.py:1208` writes `out_dir/checkpoints/latest.txt` with checkpoints under `out_dir/checkpoints/<tag>/model`. `train_stage3.py`'s resume path reads exactly that and has always been right. `autoinit_phase_a_driver.py:736-738` reads `trained/latest.txt` and `trained/<tag>/model`, **dropping the `checkpoints/` component in both**.
- **Why no `$0` gate could have caught it.** The line runs once per probe, *after* the driver's `train_stage3.py` subprocess returns and `PROBE_TRAINED` is marked. No `$0` path trains a probe — it is ~62 GPU-minutes — and the pod simulator and pre-flight rehearsal stub the training subprocess rather than producing a checkpoint tree, so this read has never been executed against real training output. Same shape as attempt 3's defect: an unexecuted line on a path only a paid pod reaches, but at the far end of the run instead of in setup.
- **A second, separable loss worth a decision.** `checkpoints_fetched` is empty and `required_products_secured` says *"stage 1 did not stage any selected leaves"* — the fetch spec collects finalists, and a trained-but-unscored rung-1 probe is not one. **62 paid GPU-minutes of training were discarded with the pod.** Whether such a probe should be journalled or fetched is a maintainer decision; it was not changed unasked.
- **Alternatives considered:** (a) fix the driver's two paths to include `checkpoints/`; (b) additionally retain a trained-but-unscored probe so a later failure does not discard the training; (c) make the probe read go through one shared helper with the resume path, so a third consumer cannot drift again. (a) is the minimum; (c) is the durable form of it. None applied — the repair is inside the 22-file harness and moves the digest as well as the session commit, so it is the maintainer's call.
- **Revisit when:** a repair is chosen and a new one-use grant issued. `$215.6837` of the `$234.00` cap is used; `$18.3163` remains, still one continuation ceiling and not a Phase-A attempt. **Balance is not authorization.**
...
- **The portability repair worked.** Setup passed for the first time in any continuation, at `$0.21`, and the driver started confirmed by descriptor probe rather than by the SSH channel returning.
- **Stage 0 and Stage 1 PASSED on hardware — the real result.** Stage 0 attested interval `0.011695`, floor `0.3000`, plan `02be33b9`. Stage 1 imported the five Attempt-12 leaves in the frozen selected order and **re-identified every one from the bytes that arrived on the pod**, matching the Stage-1 artifact and shard digests (`config_hash 567d32789ba6`); the canonical control was measured once on `state_eval@v1` over 74022 positions (`artifact_digest dc9500d3`); admission accepted the five plus the control. **No search ran and none was reachable.** The entire import path — transport, staging, strict byte re-identification, control measurement, admission — is now demonstrated end to end on a paid pod.
- **Decision: fail closed at the Stage-2 OOM.** Pod deleted at 24.9 min / **$0.4112** with provider confirmation; watchdog ended `pod_gone` after 26 ticks; evidence collected through the supported path first. No repair on the live pod, no relaunch.
- **Cause — two compounding defects, either of which alone was survivable.** (1) `release_to_subprocess(drop=[teacher, evaluator])` reported **`freed_allocated_bytes: 0`**: allocated stood at 8,110,229,504 B before *and* after, only reserved cache returned, and the handoff correctly recorded `live_retention: true` — *"a genuine retention, not allocator caching"*. Something still holds references to those tensors after `drop=` and `del`. (2) `require_headroom` demands `RECOVERY_TRAINER_BYTES + margin` = 22 + 2 = **24.00 GiB** and saw **36.32 GiB** free, so it passed with 12.32 GiB of apparent slack — while the probe used **36.30 GiB** and OOM'd asking for 298 MiB more, so it needs **≳36.6 GiB**. Had the release worked, ~43.87 GiB would have been free and the probe would have fit; had the threshold matched the trainer, the gate would have refused at `$0.36` with a diagnosis.
- **The uncomfortable part.** `require_headroom` exists *because of* attempt 12, and its refusal message names attempt 12. It did not fire. A gate whose threshold is a written constant rather than a measured one can be present, correct in form, cited in prose, and still not protect the thing it was written for. **The retention shrank from ~24.05 GiB to 7.55 GiB and the probe still did not fit, because nobody ever measured what the probe actually wants.**
- **Alternatives for the repair, not applied:** (a) find and drop whatever retains the teacher/evaluator so the release actually frees; (b) calibrate `RECOVERY_TRAINER_BYTES` from a measured rung-1 probe; (c) both. (c) is almost certainly right — (a) alone leaves a gate that would still wave through a genuinely tight card, and (b) alone wastes 7.55 GiB on every run. Both touch `autoinit_phase_a_driver.py` and `device_handoff.py`, which **are** in the 22-file harness, so unlike attempt 3's repair they move the continuation harness digest as well as the session commit.
- **Risks:** the measurement in (b) is itself a paid observation unless it can be derived from the probe's shapes and optimizer state, or from attempt 12's evidence. Deciding that cheaply is the next question, not more launches.
- **Revisit when:** the maintainer chooses among (a)/(b)/(c) and issues a new one-use grant. `$214.3326` of the `$234.00` cap is used; `$19.6674` remains, which still covers one continuation ceiling and not a Phase-A attempt. **Balance is not authorization.**

## 2026-08-22 — The attempt-3 portability repair, and the second defect it uncovered

- **Context:** the maintainer approved option (a) only — make the round-trip temporary directory use the configured scratch environment when available and otherwise fall back to the platform `tempfile` directory, requiring no dev-box path. Explicitly **not** approved: adding the module to `TEST_IGNORES`, and adding a generic hard-coded-host-root structural guard. Only a targeted regression was requested.
- **Decision:** `scratch_dir()` returns a *preference, never a requirement* — `AAD_SCRATCH` if it names an existing directory, else `/home/ecs-user/aad-scratch` if present, else `None` so `tempfile` chooses. `verify()` passes it straight to `mkdtemp`. Dev-box behaviour is unchanged, verified directly.
- **Running the fix under real pod conditions found a second instance of the same defect, and this is the part worth remembering.** With `mkdtemp` fixed, the module failed one line further down: `token()` read `~/.cache/huggingface/token`, which no pod has — every pod-side script in `autoinit_preflight_setup.sh` reads `HF_TOKEN`, exported *before* the test gate runs. It is evaluated as an **argument** to `hf_hub_download`, so it executes even in the tests that patch the download away, and it was invisible while `mkdtemp` failed first. `token()` now reads the environment first and falls back to the file. **Had I stopped at "11 passed on the dev box", attempt 4 would have died at the same gate with a different traceback.**
- **A third test could never have passed on a pod.** `test_the_before_manifest_refuses_a_drifted_canonical_shard` drove `build_before()` against the real 5.55 GiB canonical store, so on a host without it the run exited at "canonical leaf missing" and the drift assertion never ran. It now builds a synthetic store and record. Not a weakening: mutation M6 deletes the drift check and the test still fails.
- **Scope note.** The token fix and that test rewrite go beyond the literal instruction, which named only the temporary directory. They are in scope for the instruction's *stated acceptance criterion* — that the tests remain enabled and pass on a pod-like host without the dev-box directory — which neither the scratch fix alone nor the unchanged test could have met. Both were found by measurement, not by looking for more to do. No `TEST_IGNORES` entry and no generic guard were added.
- **Verification.** Full suite **2162 passed / 12 skipped** (+2 = the regression and its configured-root branch; skips unchanged). The publisher module **11 passed on a pod-like host** — a mount namespace with the repo and interpreter over a tmpfs `/home/ecs-user`, so no `aad-scratch`, no `aad-artifacts`, no token file, `HF_TOKEN` set as a pod sets it. Frozen verifier passed. **6 mutations**, each a passing state made to fail.
- **The continuation harness digest is unchanged at `162c09ed…`** — the publisher and its tests are outside the 22-file set — so this repair moves the **session commit** and nothing else. Attempt 4 still needs a fresh grant and authorization, because the lineage gate binds the base commit.
- **Revisit when:** attempt 4 is authorized. Science plan, the five selected leaves and their order, the continuation protocol, the recovery stages and the $16.7456 ceiling are all unchanged.

## 2026-08-22 — Attempt 3: the transport is proven on a pod, and a dev-box path in a dev-box tool killed the gate
...
- **Risks:** the fix is one line, but the *class* is the third distinct "a `$0` path is not the pod" failure. A cheap structural guard — a test that greps the non-harness tool modules for hard-coded host roots, or a pod-condition run of the suite — is worth more than the fix.
- **Revisit when:** the maintainer decides between (a), (b) or both, and issues a **new** one-use grant. Attempt 3's grant and authorization are spent. `$213.9214` of the `$234.00` cap is used; `$20.0786` remains, which covers one continuation ceiling and not a Phase-A attempt. **Balance is not authorization.**
...
- **What was retired, and what that means.** Five LFS objects, **8.4752 GiB**: `e5_start/p2_ceheavy_sa` and `_sb` weights (2.220 GiB each), `e8_inputs_20260810/stage0/…/activation_stats.safetensors` (1.814), `e8_init_20260810/e8_contribution_init_v1/…/model.safetensors` (1.110), and the partial `cca699c9…` leaf in the transport repo (1.110). **These are remote retention changes, not retirements of science:** every checkpoint continues to exist canonically on local disk with unchanged identity and hashes. Record: `logs/autoinit_relay_retention_20260822.json`.
- **Method, because the obvious one does not work.** Ordinary deletion reclaims nothing on this account — measured twice — since LFS is billed including history. The reclaim is `permanently_delete_lfs_files(rewrite_history=True)` on exact `file_oid` identity, never by prefix. Measured result: inventory **92.7330 → 84.2578 GiB**, exactly the objects' bytes, leaving **8.8722 GiB free** against the ~93.13 limit — inside the 8–10 GiB target. `repo_info.usedStorage` still reported the old figure, which is the documented lag; the inventory sum is the billing basis.
- **Every check re-derived at delete time**, not trusted from the analysis table: canonical local copy present under `/home/ecs-user/aad-artifacts`, local sha256 == remote `file_oid`, no current session manifest declaring the path, and not on the protected list.
- **Two copies were consolidated first.** `stage0/activation_stats` and `e8_contribution_init_v1` had their only local copy in the repository's gitignored `artifacts/` tree — a working tree the pod simulator manipulates, not an archive. Both were copied to `/home/ecs-user/aad-artifacts/relay_retired/<remote path>` and hash-verified **before** anything was deleted, so the maintainer's "canonical local copy" condition is literally true rather than nearly true.
- **What could NOT be retired, and it is most of the candidates.** 32 objects totalling **69.671 GiB** have no byte-identical local copy and are therefore the only surviving copies. `e1_scaling_20260801` alone is **42.19 GiB across 19 objects** — the largest and most obviously obsolete group by name, and untouchable for exactly the reason that makes it tempting. `stage3/*` adds 26.6 GiB. The 8.47 GiB reclaimed is genuinely all the duplicated bytes there were.
- **A near-miss worth recording.** The two wheelhouses (7.4 GiB) are fetched by `snapshot_download` inside the setup script, **not** through the `RelayInput` contract, so a rule of "delete anything no session manifest declares" would have marked them deletable. Removing them would have put PyPI back on the paid critical path — the failure that cost $2.07 across two attempts. They are on the protected list by prefix, and the protected list is re-derived inside the deletion script rather than passed in.
- **Preserved deliberately:** the small config/tokenizer/generation_config sidecars of retired checkpoints remain on the relay as reconstruction and accountability metadata. Only weight objects were removed.
- **Post-checks:** all six sessions' declared relay inputs resolve; wheelhouses (91 + 196 files), permanent controls (18) and canonical init (12) intact.
- **Revisit when:** the five-leaf mirror is verified and a continuation is authorised. Nothing was launched.

## 2026-08-22 — The Hugging Face private-storage limit is ACCOUNT-WIDE, not per-repository

- **Context:** the maintainer directed that the five Attempt-12 leaves reach a pod through a second private HF repo, citing an existing project measurement — 2026-08-13 recorded that "a 1 MiB write to a different private repo succeeded, so the limit binds per-repo, not account-wide". That premise is **wrong**, and this record supersedes it.
- **What happened.** A dedicated private repo `AlphaAvatar/aadistill-transport` was created and the leaves uploaded in the frozen selected order. It accepted **exactly one leaf — 1.110 GiB** — and then refused with `BadRequestError: Private repository storage limit reached`.
- **The measurement.** `aadistill-transport` 1.1103 GiB (6 files, brand new) + `aadistill-artifacts` 92.1687 GiB (1162 files) = **93.279 GiB**, against the ~**93.13 GiB** limit recorded on 2026-08-13. A fresh repository received no allowance of its own; it consumed the account's remaining slack and stopped. Recorded in `logs/autoinit_leaf_transport_quota_finding.json`.
- **Why the earlier inference failed, and it is the reusable lesson.** A 1 MiB write succeeding is equally consistent with an account-wide limit that happens to have ≥1 MiB of slack — which is exactly what it had. A token-sized probe cannot distinguish "per-repo" from "account-wide"; only a write at **representative size** can. The probe was real and the conclusion drawn from it was not entailed by it.
- **Consequence: transport is still unsolved.** The leaves are 5.55 GiB. scp needs 1.99 MB/s against a dev box measured this session at **0.69 MB/s** (leaf 1 took 28.8 min, 2.9× over the 600 s per-asset timeout). The main relay has ~1.6 GiB of headroom. The second-repo route is now measured to add nothing.
- **What was built anyway, and is worth keeping.** The multi-repo contract is implemented and mutation-verified, and it is transport-agnostic: `RelayInput` carries `repo` (defaulting to the main relay, so nothing existing moved), `SESSION_RELAY_INPUTS` serializes it, the shared setup fetches each item from its declared repo and names none of its own, and the `$0` precheck groups declared inputs by repository and lists every one. Whatever transport is chosen, a session can now declare where its bytes come from. The continuation declares the leaves as relay inputs and **refuses at `$0`** while no verified transport manifest exists.
- **Nothing was deleted.** The 1.110 GiB now in the transport repo is a re-creatable copy whose canonical original is intact and verifying on the dev box. Removing it would reclaim account quota, but that is a deletion decision and is left to the maintainer.
- **Options, none taken:** raise the HF plan's private storage; free account-wide quota (which on this account means the main relay, explicitly excluded); publish the leaves to a **public** repo, if public storage is not counted against the private limit — plausible but **unmeasured**, and this record is precisely why that must be tested at representative size rather than with a token write, and it is also a disclosure decision; keep the leaves on the scp path with a size-derived rather than fixed per-asset timeout (a ~2.2 h transfer, ≈$2.20 of billing, well inside the $16.7456 ceiling) — the maintainer excluded raising the timeout; or another storage backend entirely.
- **Revisit when:** the maintainer picks a transport. Frozen science, the selected ids and order, the tokenizer, control measurement, recovery, pricing, provider resilience and the $16.7456 ceiling are all untouched.

## 2026-08-22 — Recovery continuation attempt 2: the five Stage-1 leaves have no route to a pod, $0.2389

- **Context:** the maintainer gave GO for attempt 2 under the derived $16.7456 ceiling. The full chain executed: a new one-use grant, clean base `b2f0dc5`, issuance as `autoinit.recovery_continuation.2026-08-21T2004Z` against the current harness `e5a7183a…`, an authorization-only commit `3e3f2295` differing from its base in exactly one path, bundle round-trip verified by bytes, checkout and harness digest recomputed **from the relay checkout**, and all four pre-provider `$0` gates.
- **The provider-resilience closure worked.** The readiness poll that ended attempt 1 succeeded here — TCP 22 at **3.7 min** — and `read_image_digest` confirmed the image. That failure class is closed on hardware.
- **Outcome: no stage ran.** Pod `7hthdteyc25xgx`, 14.48 min, **$0.2389**, deleted by the launcher with provider confirmation; watchdog ended `pod_gone` after 16 ticks. `TimeoutExpired` staging the first Stage-1 leaf.
- **Root cause, and it is arithmetic.** `SessionRunner` stages each declared `LOCAL_ASSET` with `subprocess.run(scp …, timeout=600)`, and `subprocess.run` **raises** on timeout. One leaf is **1.110 GiB (1192 MB)**, so fitting the 600 s window needs **1.99 MB/s sustained**. This session's own bundle upload, minutes earlier, ran at **0.44 MB/s**; the recorded dev-box uplink is **0.72 MB/s**. One leaf therefore needs **28–45 min against a 10-minute timeout**, over by 3–4.5×, and four more would have followed. **It could not have succeeded on any attempt.**
- **Why no `$0` gate saw it.** Each gate was true and none was sufficient: `selected_leaves_present_gate` asks whether the leaves exist and verify **locally**, which they do, and says nothing about delivery; `test_each_real_session_manifest_stages_through_the_real_block` exercises `SESSION_RELAY_INPUTS`, which the pod **pulls**, while the leaves are `LOCAL_ASSETS`, which the launcher **pushes** by scp — a different transport, uncovered; and the pod simulator never scps. The gap is between *the bytes are correct* and *the bytes can arrive*.
- **Both transports are closed.** scp needs 1.99 MB/s against ≤0.72 MB/s observed. The relay alternative is off by default and documented in the launcher: `usedStorage` 91.54 GiB against an inferred 93.13 GiB limit — **1.60 GiB of headroom against 5.55 GiB of leaves**. So the five selected leaves currently have **no route to a pod**, and that is the finding.
...
- **Revisit when:** the maintainer chooses a transport. Evidence: [`autoinit_recovery_continuation_attempt2/`](../runs/unscoped/recovery_continuation/attempt2/).
...
- **Mutation-verified 9 ways:** removing the continuation from the covered set fails; each of the six operational fields fails on a *real* collision (the first `driver_job_id` mutation used a value that did not actually collide and correctly passed — the mutation was wrong, not the test); rewriting the continuation's `plan_hash` fails; claiming `runs_a_search: True` fails.
- **Coverage gained.** The continuation now runs the same generic checks as every other paid session, including `test_each_real_session_manifest_stages_through_the_real_block` — its real relay manifest, through the real staging shell, landing the real destinations, at `$0`.
- **Risks:** none identified. Nothing in the continuation, the resilience logic, the budget, the imported leaves, control measurement or recovery execution changed; the only production file touched was none — the change is confined to the two test files.
- **Revisit when:** a future session shares an operational field for a defensible reason, which would need the same treatment this one got: name why, and assert it.

## 2026-08-22 — Control-plane resilience: all three SessionRunner reads, not only the one that failed

- **Context:** recovery-continuation attempt 1 died on `URLError: SSL UNEXPECTED_EOF` in `wait_endpoint`, 27 s into a billing pod, against a control plane measured at **25% transport failure**. The maintainer accepted the diagnosis and directed that the fix cover **all three** direct `provider._gql()` call sites in `SessionRunner` — because `read_image_digest()` also runs after the pod exists and is billing, so patching only the endpoint poll would move the same failure one step later.
- **Decision:** name the classification once and route every launcher read through it. `provider.TRANSIENT_TRANSPORT` is the tuple `get()` has always caught (`URLError`, `OSError`, `ValueError` — `JSONDecodeError` is a truncated body — and `TimeoutError`); `provider.observe()` applies it to any query and returns an `Observation` that **never raises**, with a GraphQL `errors` body classified as *the server declining to answer*, never as an answer of "no".
- **Per site.** `check_gpu_offered` runs at `$0` before any pod exists: it retries on the launcher's existing pod-acquisition budget (`--create-attempts` × `--create-retry-seconds`) and then aborts cleanly — no pod created, nothing propagating through the launcher, and an unanswered price query reported as unknown rather than as "GPU not offered". `wait_endpoint` keeps polling under the **caller's** deadline; a failed observation costs exactly one poll interval and is never read as no-ports or as gone. `read_image_digest` retries under the *same* deadline and then **fails closed** with `ImageIdentityUnavailable`, which `setup_on_draw` turns into `no_image_identity` — teardown before setup runs.
- **`setup_on_draw` now owns one startup deadline** for both billing observations. Previously each method derived its own `startup_limit_min`, so the operator's configured bound could silently cover two windows. No new timeout constant was added and no deadline value changed.
- **`no_image_identity` is deliberately not redrawable.** The control plane failed, not the host; redrawing would pay again for the same unanswered question. And the old `self.a.image` fallback is gone: the requested image is what we asked for, not what is running, and an unverified identity must never enter a reproducibility record.
- **Alternatives considered:** relaunch unchanged (rejected — at 25% loss, surviving five polls is ~24%, so it repeats rather than gambles); fix only `wait_endpoint` (rejected by the maintainer for the reason above); add a dedicated retry budget (rejected — it would be a new timeout constant, and existing knobs already express both bounds).
- **Expected upside:** the failure class that cost attempt 1 cannot end a session at any of the three sites, and cannot silently return.
- **Risks:** a genuinely dead control plane now costs up to `startup_limit_min` of billing before the session gives up, instead of failing in 27 s. That is the intended trade — the same bound the operator already configured for startup — and it fails closed at the end of it.
...
- **Context:** the maintainer gave an explicit GO for exactly one Phase-A recovery continuation under the derived $16.7456 hard ceiling, against tested implementation `03c2574`. The full eight-step chain was executed: a one-use grant with `grant_type: recovery_continuation`, clean pre-authorization base `b1ebbb6`, issuance as `autoinit.recovery_continuation.2026-08-21T1642Z` against the continuation harness digest `f2ea4332…` over 22 files, an authorization-only commit `8c7c42e` differing from its base in exactly one path, bundle upload with byte, checkout and harness round-trip verification **from the relay copy**, and all four pre-provider $0 gates.
- **Outcome: no stage ran.** Pod `dckc72mtoe9ijw`, 0.7 min, **$0.01**, deleted by the launcher with provider confirmation (`provider_confirms_gone: true`, and the account shows no pods). Terminal state `INCOMPLETE` with `launcher_error`. **No leaf was read on the pod, and no science changed.**
- **Root cause, measured at $0 after the fact.** The RunPod GraphQL endpoint was failing **5 of 20 requests — 25%** with `SSL: UNEXPECTED_EOF_WHILE_READING`, `ECONNRESET` and `RemoteDisconnected`. `session_runner.wait_endpoint()` polls that endpoint every 10 s for up to 15 minutes — up to **90 requests** — by calling `provider._gql()` **directly**, and catches nothing. `provider.get()` is the wrapper built for precisely this case and carries the comment *"Never raises. A watchdog that dies on a transient 502 is not a backstop."* The launcher was never given the same tolerance.
- **Blast radius is bounded, and worth recording precisely.** Three `_gql` call sites are uncaught, all pre-driver: `check_gpu_offered` (1 call, before any pod exists, $0), `wait_endpoint` (≤90 calls, pod already billing — this failure), `read_image_digest` (1 call, after SSH). **The 15-hour main poll loop uses `provider.get()`** and is not exposed; a blip mid-run does not kill the session.
...
- **Revisit when:** the maintainer decides on the `wait_endpoint` tolerance fix and a new one-use grant. Evidence: [`autoinit_recovery_continuation_attempt1/`](../runs/unscoped/recovery_continuation/attempt1/).

## 2026-08-14 — recovery_search_v1 is INVALID; v2 is its semantic successor

**v1 is invalid before first use and is preserved unmodified.** Its `tools` field retained the xLAM source serialization — a JSON string — instead of the project's canonical tool representation, so `apply_chat_template` could render **0 of 20** tool prompts. It never produced a valid rollout, and **no historical recovery-search result is being rewritten, because no valid v1 tool characterization ever existed.** Marked in place by a sibling `SUPERSEDED.md`; not one byte of the asset was edited.

- **v2 is the narrowest change that can fix it.** Identical membership, ordering, item ids, messages, prompts, tool names, descriptions, parameter maps, gold calls, scorer inputs and non-tool items; identical `recovery_search_scoring@v2`. **Only the materialized `tools` value changes**, through `aadistill.data.tools.xlam_tools_to_canonical` — the same converter `build_stage2_v1.build_xlam` already used for the training mixtures, now factored into one function with two callers rather than copied. The battery therefore shows the model the tool form the student was trained on.
- **`content_sha256` is identical in v1 and v2** (`a1b22778…`) and the builder asserts it. That hash covers `id:prompt_sha256` pairs, so its equality *is* the machine-checkable proof that membership, ordering, ids and prompts were untouched. The assets are distinguished by a new `tools_materialization_sha256` (`3016f1c4…`) and by their manifests (`72d8c053…` → `58ae5c6d…`).
- **Full $0 structural audit, all 20 items** (`scripts/autoinit/audit_recovery_search_v2.py`, artifact `autoinit_recovery_search_v2_audit.json`): tool count, names in order, descriptions, whole parameter maps, parameter order, types, defaults and the required semantics derived from them — all preserved; the six non-tool sets byte-identical; **20/20 render** under the canonical evaluator, with the model-visible prompt and token-id hashes frozen.
- **The audit rejected the first v2 build.** Writing rows with `json.dumps(sort_keys=True)` alphabetised the keys *inside* `tools`, reordering parameter names — invisible to dict equality, visible in the rendered `<tools>` blob, and therefore a change to what the model is shown. In v1 `tools` was a string so its interior had never been touched. The builder now sorts only top-level keys, and the audit checks parameter order on its own.
- **The migration exposed a real scoring defect, and only then was the scorer touched.** `score_recovery_search.py` fed `sample["tools"]` to `as_openai_tools`, which reads xLAM-shaped entries; under the canonical envelope the name lives at `t["function"]["name"]`, so every declared name resolved to `None`, `tool_name_valid` was **0.0 for every item**, and the tool capability was structurally zero — the same class of defect that produced `recovery_search_scoring@v2` in the first place. `validate_recovery_scoring.py` refused to launch on it. The scorer's input is now normalized through `normalize_tools`, which reads either representation and still fails closed on malformed JSON, a non-list root, an empty list or an unnamed entry.
- **Scoring semantics are unchanged, and that is measured.** After the fix, `validate_recovery_scoring.py` over nine policies × 190 prompts reproduces **every number** of the pre-migration record. The contract stays `recovery_search_scoring@v2`; its digest is re-pinned `69591aab…` → `799398e7…` because the files moved.
- **Identity graph updated consistently:** the setup gate's preregistered constants (now including `tools_materialization_sha256`, recomputed from the items rather than read from the manifest), the scorer's battery pins, the driver, launcher and setup staging, the role-isolation and preregistration writers, `RecoveryEvaluationProtocol`'s battery identity, and the preflight plan hash (`37dbd7b2…` → `afd08be7…`, moved only because the plan's Stage-3 text names the battery). Preregistration re-emitted: 25 changed leaves, every one an identity digest, battery pointer, plan hash or timestamp — no threshold, rule, seed, ranking policy or capability definition moved.
- **The two permanent controls are untouched and must not be retrained.** Recovery identity and evaluation identity are separate: the controls are bound to `RecoveryProtocolFingerprint aad75fee8a897d9c…` and their own weights, and no tool prompt was involved in producing them. Only their Stage-3 characterization is outstanding.
...
- **The controls.** sa and sb, 1023/1023 steps, 61.6 min each, both reconstructing **40/40 material protocol fields from their own artifacts** — 0 mismatched, 0 unknown, pack hash recomputed, initialization independently hashed to the pinned `86fbba78…`, seeds observed as 20260726 / 20260801. Observed protocol `aad75fee8a897d9c…`, identical to the Stage-0 attestation and to attempt 3's, because both ran on the same image and host driver. Probe ids `799bd5ac…` / `793f786a…`. Weights `573847a730c1a499…` / `4c6adcf861871690…`, **re-verified against the fetched files after transfer**, stored under `/home/ecs-user/aad-artifacts/autoinit/`.
- **Bitwise reproduction is not expected and did not occur.** Attempt 3's controls had the same protocol, initialization and seeds, and different weight hashes. Training on a GPU is not bitwise reproducible; this is why the project selects on measured behaviour rather than on weight identity.
- **The attestation demonstrably tracks the host.** Attempt 2 attested `6b9b3da9…` on image digest `@580.159.03`; attempts 3 and 4 attested `aad75fee…` on `@580.126.09`. Same image tag, different NVIDIA driver, different runtime — which is exactly what the image digest is in the identity for.
- **Stage 3 is blocked, and now diagnosed.** `uncapped_eval.py` line 321 passes the frozen battery's `tools` field straight to `apply_chat_template`, and **transformers 5.15** raises `ValueError: Tools should either be a JSON schema, or a callable function…`. The battery stores tools as a **JSON string** of xLAM-style objects whose `parameters` is a flat name→spec dict, not a JSON Schema. Historical E1-E7 tool evaluations ran under transformers 4.x, which accepted that form.
- **This is left open deliberately.** Normalizing the tools changes what the model is shown in every tool prompt, which changes the `tool` capability the `recovery_search_scoring@v2` work exists to measure, and makes new numbers incomparable to the historical ones rendered under 4.x. The options — normalize at the generator, normalize in the frozen asset, or pin the evaluation transformers version — are a protocol decision with a scientific consequence, not a mechanical fix, and the session had $1.86 of authorization left. **Recorded and stopped rather than chosen.**
- **The smoke found nothing because it looked away.** It ran `sorted(sets)[0]` = `code`, the one set that declares no tools. It now runs the `tool` and `rag` sets: the hardest inputs, not the alphabetically first. A smoke that avoids the awkward case agrees with you for free.
- **Cost:** $3.60, of which ~$0.41 was a cold first draw that the launcher abandoned and deleted correctly. Micro-preflight cumulative **$6.7369 of the $8.60 authorized**; $1.86 remains, which is less than a session costs, so nothing further is launched.

## 2026-08-14 — Both controls trained, verified, and then deleted (attempt 3, $2.82)

**The verification worked. The retention did not.** Two permanent canonical controls were trained on pod `6xrudht3i7wc62`, both passed the strict observed-protocol check, and both were destroyed by the launcher ten minutes later. This entry records three defects, in cost order.

- **What was produced and lost.** sa and sb, 1023/1023 steps each, 61.6 min each. Both reconstructed **40/40 material protocol fields from their own artifacts**, 0 mismatched, 0 unknown; pack hash recomputed from the pack each run read; observed protocol identical to the Stage-0 attestation (`aad75fee8a897d9c…`); seeds observed as 20260726 / 20260801; initialization independently hashed to the pinned `86fbba78…`. Probe ids `799bd5ac…` / `793f786a…`, weights `5682d0b9…` / `1ccf4b52…`. **The record of what they were survives in `preflight_evidence.json`; the weights do not.**
- **Defect 1 — the checkpoint fetch was gated on total success.** `if terminal == "ALL_DONE"`, directly under a comment reading "the permanent controls are the only artifacts that cannot be regenerated without paying again". Stage 3 is non-blocking *precisely because* the controls still exist and are kept — and `PREFLIGHT_INCOMPLETE` therefore skipped the fetch and deleted the pod. Now gated on Stage 2 having passed, and the fetch reports transferred bytes rather than a return code alone.
- **Defect 2 — no preflight session could ever collect its artifacts.** Both spec files are documents with an `entries` list whose items carried a `final` key; `load_specs` iterated the *document* (yielding its keys) and `ArtifactSpec` has no `final` field. Every session died with `manifest rc=1` and kept only the two files the launcher scp's by name. That is why the run manifests, completion records, train logs, probe-identity records and the Stage-3 generation tail are gone. Reproduced offline in seconds; the loader now accepts both shapes and fails with a legible message, both specs are rewritten against the real `ArtifactSpec` schema, and a test loads both.
- **Defect 3 — Stage 3's generation failed and the reason is unrecoverable**, a direct consequence of defect 2. `preflight_ctl_r0860k_sa generation rc=1` is the whole of what survives.
- **Decision — the generation path is smoke-tested inside Stage 1.** It is the one path with no cheap rehearsal anywhere: the evaluator needs vLLM and a GPU, so no CPU test can execute it. Stage 1 now runs the entire path — `uncapped_eval.py`, the summaries it writes, `observe_generation_protocol`, and the comparison against the attested fingerprint — on **two prompts against the canonical init**, before any control is trained. A failure now costs profiling minutes; on 2026-08-13 it cost $2.69 of controls and told us nothing.
- **The pattern across all three attempts.** Every failure has been in code that had never executed: an interpreter path, three measurement scripts, an artifact spec, a fetch condition. The suite is large and the harness is heavily rehearsed, but rehearsal that stubs the thing being rehearsed proves the caller, not the callee. What changed today is that each of those now runs for real, cheaply, before the money.
- **Cost:** $2.82. Cumulative $183.84 of the $211.07 cap. Micro-preflight to date: $3.1369 of the $8.60 authorized.

## 2026-08-14 — The Stage-1 probes had never been run (micro-preflight attempt 2, $0.29)
...
- **Cost:** $0.2869. Cumulative $181.0202 of the $211.07 cap; $30.0498 uncommitted. Two attempts, $0.3169 total, no scientific artifact produced and none corrupted.

## 2026-08-13 — A gate that cannot run is not a gate (micro-preflight attempt 1, $0.03)

- **What happened.** Pod `ejewm772b02air`, L40S at $0.99/h, created 15:31:52 UTC and deleted 1.5 min later by the launcher, provider-confirmed gone. Setup reached `REPO_READY`, staged and hash-verified the canonical init and both copies of the pack, then died at the frozen-asset gate with `SETUP_RC=91`. **No stage ran; nothing was trained, generated or measured.**
- **The defect.** The gate invoked `/opt/train/bin/python`, which `uv sync` creates ~60 lines *later*. It could never have run. Worse than the ordering: the `||` branch printed "FROZEN ASSET IDENTITY MISMATCH -- not the preregistered assets" for a missing interpreter, so the one artifact a reader would have had asserted a cause nobody established, and pointed at the frozen assets — which were fine.
- **Decision (1) — the gate moves to just after `TRAIN_ENV`.** It needs `/opt/train` because the scoring-contract digest comes from `aadistill.autoinit.recovery`, and importing that package imports torch. It still runs inside setup, so it still blocks before Stage 0 and before any scientific measurement, which is the requirement. The cost of the move is that a frozen-asset mismatch is now found after `uv sync` rather than before it; the two large pinned inputs are still byte-checked early by the inline stdlib block, so what is deferred is the state_eval / recovery_search identity check.
- **Decision (2) — the failure prints the verifier's own output verbatim.** "The verifier could not run" and "these are not the preregistered assets" are different findings and must not be reported as the same one.
- **Decision (3) — the class is tested, not the instance.** `test_setup_never_invokes_an_interpreter_it_has_not_built_yet` asserts that no line uses a venv interpreter before the line that builds that venv, for both venvs. A test that only pinned this one call site would have let the next one through.
- **Why no local check caught it.** `simulate_pod_env.sh` runs the *test suite* under a pod-like artifact set; it has never run the setup script, and cannot — the setup builds venvs and downloads a teacher. The generalized ordering test is the cheap substitute: it reads the script rather than running it.
- **Cost:** $0.03. Cumulative spend $180.7333 of the $211.07 cap; $30.3367 uncommitted. The authorization was re-issued against the corrected harness, which is the pin behaving as designed.
...
- **Revisit when:** the five zero-cost prerequisites in [`autoinit_pilot_proposal.md`](../experiments/shared/analyses/autoinit_pilot_proposal.md) §3 are met. The statistics-pass GPU/CPU split remains the only reason every cost is a range.
...
- **Revisit when:** the prerequisites in [`autoinit_pilot_proposal.md`](../experiments/shared/analyses/autoinit_pilot_proposal.md) §3 are met — the state-evaluation suite and recovery search battery are built and leakage-checked, the halving plan is frozen, and the statistics-pass GPU/CPU split is measured. That last one is the only reason every cost in the proposal is a range.
...
- **What this intentionally destroys:** every commit revision previously recorded
  in [`artifact_manifests.md`](../state/artifact_manifests.md) — `b955bd2f…` (stage1
...
- **Context:** E6b completed with operational deviations
  ([`e6b_protocol_deviations.md`](../experiments/early/analyses/e6b_protocol_deviations.md)). The maintainer
...
- **Decision:** freeze the backend as **PyTorch SDPA using its flash backend (already
  ...
  Full audit: [`e8b_backend_audit.md`](../experiments/early/analyses/e8b_backend_audit.md).
...
- **Measured:** one depth invocation over the **complete** 59,763-position
  ...
  ([`autoinit_phase_a_full_mixture_depth.json`](../experiments/phase_a/analyses/autoinit_phase_a_full_mixture_depth.json)).
...
- **The traceback preservation paid for itself on its first run.** The whole
  frame is in
  [`autoinit_phase_a_attempt7/stage1_traceback.log`](../runs/unscoped/phase_a/attempt7/stage1_traceback.log),
...
- **Rule adopted: one owner per fact.** A file that does not own a fact links to
  ...
  [`logs/CATALOG.md`](../state/ownership.md) with five classes — CURRENT, REFERENCE,
...
[`autoinit_measurement_attempt2/`](../runs/unscoped/measurement/attempt2/).
...
**$12.5259**. Evidence:
[`autoinit_measurement_attempt3/`](../runs/unscoped/measurement/attempt3/).
...
Evidence: [`autoinit_phase_a_attempt11/`](../runs/unscoped/phase_a/attempt11/).
...
[`autoinit_phase_a_attempt12/`](../runs/unscoped/phase_a/attempt12/).
